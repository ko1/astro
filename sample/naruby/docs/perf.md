...
|--------------|------:|------:|--------:|----------:|--------:|---------:|-------:|----------:|--------:|-------:|-------:|-------:|-------:|
| fib          |  9.12 |  1.12 |    4.16 |      0.71 |    0.63 |     4.59 |   0.57 |      4.69 |    0.57 |   0.46 |   0.40 |   0.12 |   0.14 |
| ackermann    |  6.29 |  0.73 |    6.04 |      0.83 |    0.77 |     5.32 |   0.78 |      5.49 |    0.75 |   0.53 |   0.31 |   0.05 |   0.06 |
| tak          |  1.20 |  0.20 |    0.62 |      0.20 |    0.13 |     0.68 |   0.13 |      0.70 |    0.13 |   0.06 |   0.05 |   0.04 |   0.02 |
| gcd          |  4.79 |  2.09 |    3.34 |      0.24 |    0.18 |     3.27 |   0.19 |      3.50 |    0.17 |   0.22 |   0.15 |   0.15 |   0.15 |
| loop         |  0.88 |  0.88 |    0.86 |      0.05 |    0.00 |     0.91 |   0.00 |      0.92 |    0.00 |   0.04 |   0.02 |   0.00 |   0.00 |
| call         | 18.44 |  5.34 |    5.92 |      1.72 |    1.59 |     6.39 |   1.25 |      5.87 |    0.91 |   1.09 |   1.07 |   0.31 |   0.32 |
| chain20      | 19.81 |  3.97 |    5.25 |      1.86 |    1.75 |     5.86 |   1.37 |      6.28 |    1.03 |   1.27 |   0.97 |   0.27 |   0.27 |
| chain40      | 20.80 |  3.83 |    6.53 |      3.20 |    2.95 |     6.79 |   3.02 |      6.97 |    1.12 |   2.49 |   2.24 |   0.25 |   0.25 |
| chain_add    |  2.11 |  0.65 |    0.88 |      0.27 |    0.17 |     1.05 |   0.12 |      1.09 |    0.09 |   0.11 |   0.10 |   0.03 |   0.03 |
| compose      |  2.37 |  1.26 |    1.04 |      0.20 |    0.13 |     1.17 |   0.14 |      1.15 |    0.08 |   0.10 |   0.09 |   0.09 |   0.09 |
| branch_dom   |  2.25 |  1.72 |    1.30 |      0.21 |    0.15 |     1.36 |   0.14 |      1.43 |    0.14 |   0.07 |   0.07 |   0.07 |   0.07 |
| deep_const   | 17.52 |  5.18 |    4.35 |      1.64 |    1.54 |     4.67 |   1.31 |      4.90 |    0.82 |   1.01 |   0.99 |   0.32 |   0.32 |
| collatz      |  5.38 |  0.87 |    3.71 |      0.23 |    0.16 |     3.76 |   0.16 |      3.96 |    0.15 |   0.34 |   0.15 |   0.14 |   0.14 |
| early_return |  5.77 |  0.70 |    4.39 |      0.35 |    0.28 |     4.09 |   0.29 |      4.50 |    0.28 |   0.30 |   0.28 |   0.28 |   0.28 |
| prime_count  |  9.30 |  3.46 |    6.88 |      0.51 |    0.44 |     7.18 |   0.44 |      6.83 |    0.44 |   0.49 |   0.43 |   0.43 |   0.43 |
...
| `n/aot-1st` | cold で `./naruby --ccs -c bench/$B.na.rb`。`--ccs` で code_store/ クリア、`-c` で AOT bake (SDs only)、その後 SDs で run。compile + run の合計 |
| `n/aot-c` | aot-1st 後 (= all.so に SD あり) で `./naruby -b bench/$B.na.rb`。bake skip、cs_load で SD を bind して run |
| `n/pg-1st` | cold で `./naruby --ccs -p bench/$B.na.rb` (**`-c` は意図的に渡さない**)。`-p` のみ → parser が pg_call_<N> emit、SD は bake せず interp で run、終了時に **PGSDs だけ** を bake。**plain interp + PGSD compile** の合計 (= 数秒) |
| `n/pg-c` | pg-1st 後で `./naruby -p -b bench/$B.na.rb`。bake skip、cs_load で **PGSDs only** を bind して run。**本物の PGO 効果が見える列** |
| `n/lto-1st` | pg-1st と同じだが LTO env (`ASTRO_EXTRA_CFLAGS=-flto`、`ASTRO_EXTRA_LDFLAGS="-Wl,-Bsymbolic -flto"`) |
| `n/lto-c` | lto-1st 後で `./naruby -p -b`。LTO baked PGSDs で run |

> **`-c` を pg-1st に渡さない理由**: `-c` を入れると AOT SDs (HORG-keyed)
> も all.so に焼かれて、PGSDs (HOPT-keyed) と混ざる。cs_load の検索で
> 余計なノイズになるので、**pg は PGSDs only にしておく**。代償として
> pg-1st の run portion は AOT 速度ではなく interp 速度になるが、それは
> 「PGO calibration run」の特性として許容する。pg-c が PGSDs だけを使
> って run するので、`pg-c / aot-c` で「PGO-baked direct call vs AOT
> indirect」の差がそのまま見える。
...
|-------|------:|------:|------------:|
| fib          | 0.57 | 0.14 | 4.1× (gcc 勝) |
| ackermann    | 0.75 | 0.06 | 12.5× (gcc) |
| tak          | 0.13 | 0.02 | 6.5× (gcc) |
| gcd          | 0.17 | 0.15 | **1.13× ≈ tie** |
| loop         | 0.00 | 0.00 | DCE で比較不能 |
| call         | 0.91 | 0.32 | 2.8× (gcc) |
| chain20      | 1.03 | 0.27 | 3.8× (gcc) |
| chain40      | 1.12 | 0.25 | 4.5× (gcc) |
| chain_add    | 0.09 | 0.03 | 3.0× (gcc) |
| compose      | 0.08 | 0.09 | **0.89× (n 勝)** |
| branch_dom   | 0.14 | 0.07 | 2.0× (gcc) |
| deep_const   | 0.82 | 0.32 | 2.6× (gcc) |
...
|-------|-----:|------:|------:|------:|----------------------:|
| fib          | 4.16 | 6.6×  | 1.11× | 1.0×  | **7.3×** |
| ackermann    | 6.04 | 7.8×  | 0.99× | 1.04× | **8.1×** |
| tak          | 0.62 | 4.8×  | 1.0×  | 1.0×  | **4.8×** |
| gcd          | 3.34 | 18.6× | 0.95× | 1.12× | **19.6×** |
| loop         | 0.86 | ∞ (DCE) | — | — | (DCE) |
| call         | 5.92 | 3.7×  | 1.27× | 1.37× | **6.5×** |
| chain20      | 5.25 | 3.0×  | 1.28× | 1.33× | **5.1×** |
| chain40      | 6.53 | 2.2×  | 0.98× | 2.70× | **5.8×** |
| chain_add    | 0.88 | 5.2×  | 1.42× | 1.33× | **9.8×** |
| compose      | 1.04 | 8.0×  | 0.93× | 1.75× | **13.0×** |
| branch_dom   | 1.30 | 8.7×  | 1.07× | 1.0×  | **9.3×** |
| deep_const   | 4.35 | 2.8×  | 1.18× | 1.60× | **5.3×** |
| collatz      | 3.71 | 23.2× | 1.0×  | 1.07× | **24.7×** |
| early_return | 4.39 | 15.7× | 0.97× | 1.04× | **15.7×** |
| prime_count  | 6.88 | 15.6× | 1.0×  | 1.0×  | **15.6×** |
...
`*-1st` は `code_store/` 空 + ccache off の状態から始まる cold run。

- **`aot-1st`** = `--ccs -c` → AOT bake (SDs) → SDs で run。`aot-1st - aot-c`
  がほぼ純粋な AOT compile + dlopen のオーバーヘッド。
- **`pg-1st`** = `--ccs -p` → **interp で run** (= `n/plain` 並み) →
  終了時 PGSD bake。run portion は plain 速度なので、`pg-1st - n/plain`
  がほぼ PGSD compile + dlopen 時間。
- **`lto-1st`** = `pg-1st` + `-flto` env。LTO link は遅いので 1st-cost
  上乗せ (chain40 で +0.5 秒程度)。

| bench | aot-1st | aot-c | aot 1st-c | pg-1st | n/plain | pg 1st-plain | lto-1st | lto-c |
|-------|------:|------:|----:|------:|------:|----:|------:|------:|
| fib       | 0.71 | 0.63 | 0.08 | 4.59 | 4.16 | 0.43 | 4.69 | 0.57 |
| ackermann | 0.83 | 0.77 | 0.06 | 5.32 | 6.04 | -0.7  | 5.49 | 0.75 |
| call      | 1.72 | 1.59 | 0.13 | 6.39 | 5.92 | 0.47 | 5.87 | 0.91 |
| chain20   | 1.86 | 1.75 | 0.11 | 5.86 | 5.25 | 0.61 | 6.28 | 1.03 |
| chain40   | 3.20 | 2.95 | 0.25 | 6.79 | 6.53 | 0.26 | 6.97 | 1.12 |
| collatz   | 0.23 | 0.16 | 0.07 | 3.76 | 3.71 | 0.05 | 3.96 | 0.15 |
| prime_count| 0.51| 0.44 | 0.07 | 7.18 | 6.88 | 0.30 | 6.83 | 0.44 |

`aot 1st-c` (AOT compile cost): **0.07-0.25 秒** とかなり安い。  
`pg 1st-plain` (PGSD compile cost): **0.0-0.6 秒** ─ AOT compile より
少し重い + plain run の測定ノイズで負になることも。  
**実用パターン**: 「速く立ち上げて PGSD も用意したい」なら 1 度
`./naruby --ccs -c -p` で **AOT 速度で run + 終了時 PGSD bake** という
合わせ技も可 (ただし all.so に SDs と PGSDs が混ざる)。bench は
混ぜずに `aot-1st` と `pg-1st` を分離している。
