...
## 5. 実行モード (orthogonal flags)

abruby と同じ直交フラグモデル: `-c` (AOT bake) と `-p` (PG bake) は
独立して有効化でき、組み合わせ自由。

```
INIT (cs_init unless --plain)
PARSE (parser uses -p flag to choose call NODE kind)
if (-c && !-b) build_code_store_aot      ← SDs のみ bake
if (!--plain)  cs_load                    ← all.so から SD/PGSD bind
               EVAL
if (-p && !-b) build_code_store_pgsd      ← AST walk → cc 反映 → PGSDs bake
```

### 5.1 Plain (`-i` / `--plain`)

`cs_init` も `cs_load` も呼ばない。`head.dispatcher` は ASTroGen 生成の
`DISPATCH_node_xxx` のまま、すべての子呼び出しは indirect call。

### 5.2 AOT (`-c`)

```
1st run (--ccs -c):   parse → build_aot → cs_load → EVAL
                                ↓               ↓
                             SD_<hash>.c → make → all.so
                                                  ↓
                                                 dlsym("SD_<hash>") → swap dispatcher
2nd run (-b):         parse → cs_load → EVAL
                                ↓
                             既存 all.so から SD bind
```

`-c` は run の **前** に bake。これにより 1st run も specialized code で
動く。`-b` は bake skip (cached run のタイミング測定用)。`--aot-compile`
は EVAL をスキップ (compile only)。

### 5.3 Profile-Guided (`-p`)

`-p` の動作は **2 段階**:

1. **parser kind 切替**: argc ≤ 3 の call site は `node_pg_call_<N>` で
   emit する (sp_body 持ち)。argc > 3 は `node_call2` (PG fallback)。
   `-p` なしは `node_call_<N>` (sp_body なし、cc-indirect)。
2. **post-EVAL bake**: EVAL が終わった後に `build_code_store_pgsd` を
   呼ぶ。AST walk で各 pg_call NODE の `sp_body` を `cc->body` (= 当該
   run で実際にディスパッチされた body) で書き戻し、`HOPT(call_site)`
   を再計算、`PGSD_<HOPT>` を出力。

```
1st run (--ccs -c -p):
  parse → build_aot → cs_load → EVAL → build_pgsd (cc 反映 PGSDs)
                                          ↓
                                       all.so に PGSDs を追加
                                       hopt_index.txt 更新
...
2nd run (-p -b):
  parse → cs_load → EVAL
            ↓
         hopt_index 経由で PGSD_<HOPT(call site)> を dlsym
         → dispatcher swap → PGSD で実行
         (= cc->body == sp_body の baked-direct call)
```

#### node_call_<N> vs node_pg_call_<N> (parser kind の違い)

| | node_call_<N> (`-p` なし) | node_pg_call_<N> (`-p` あり) |
|---|---|---|
| sp_body operand | なし | あり |
| arg operands | explicit (`a0, a1, ...`) | explicit (同上) |
| callee frame | `VALUE F[locals_cnt]` | 同上 |
| 本体 dispatch | `EVAL(c, cc->body, F)` indirect | `(*sp_body_dispatcher)(c, sp_body, F)` (SD ベイク後 direct) |
| 2 段ガード | なし (cc->serial だけ) | あり (cc->serial && cc->body == sp_body) |

`-p` は parse 時に「**この call site では PGO を効かせる**」と決める。
それ以外の場合は `node_call_<N>` で軽量 (= 2-check や sp_body 不要)。
