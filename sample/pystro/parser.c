...
{
    // Walk remaps from innermost to outermost.  For each remap:
    //   - If the remap's scope matches cur_scope, we're inside the comp
    //     itself — always apply (comp loop target should never refer
    //     back to a same-named outer binding).
    //   - Otherwise we're in a nested scope (lambda / def inside the
    //     comp body): apply UNLESS cur_scope itself binds `name`
    //     (e.g. `[lambda i=i: i for i in ...]`'s body uses the param).
    for (int i = comp_remap_top - 1; i >= 0; i--) {
        if (comp_remap_stack[i].orig != name) continue;
        if (comp_remap_stack[i].scope == cur_scope) {
            return comp_remap_stack[i].synth;
        }
        bool shadowed = false;
        if (cur_scope) {
            for (int j = 0; j < cur_scope->nlocals; j++) {
                if (cur_scope->locals[j] == name) { shadowed = true; break; }
            }
        }
        if (!shadowed) return comp_remap_stack[i].synth;
    }
    return NULL;
}
