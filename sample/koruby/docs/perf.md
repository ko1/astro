...
10 runs after 3 warmups, `taskset -c 0`, gcc -O2 (LTO なし) for the koruby
binary; SDs in `code_store/all.so` always built with `gcc -O3 -fPIC -fno-plt
-march=native` (LTO なし — 各 SD は独立 .o → `-shared` でリンク).
...
| **koruby (AOT — `make koruby-aot`)** | **69 fps** | **1.68×** |
| abruby (--aot + --pg-compile, AOT + PGC) | 75 fps | 1.83× |
| **koruby (AOT + PGO — `make koruby-pgo-aot`)** | **79 fps** | **1.93×** ← exceeds abruby +pgc |
| **ruby --yjit / --jit** | **175 fps** | **4.27×** |

(注: LTO は撤去 — incremental rebuild の体感を優先。 hot inline は
`object.h` / `prologues.h` の static-inline と SD への include で
カバー済みなので perf 影響は限定的 (PGO+AOT で LTO の inline 効果を
ほぼ回復)。 cold-tail を `korb_node_*_slow` として koruby 本体に hoist
し SD 内に複製しないことで `all.so` が 867K → 785K (-10%)。)
