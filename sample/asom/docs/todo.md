...
### 型特化ノード — AOT を効かせる主要施策 ✓ (Integer のみ)

**実装済み (2026-04-29)**: `node_send1` の Integer 版を 8 種類追加。
...
各ノードはレシーバ・引数とも SmallInteger ならインライン演算、外れたら
`swap_dispatcher(n, &kind_node_send1)` で汎用 send1 に戻して再ディスパッチ。
パース時にセレクタが `+ - * < > <= >= =` のいずれかなら最初から特化版を
発行する（warmup 不要で AOT bake が特化版を捕捉できる）。スロー
パスでも動的タイプ・フィードバックで `prim_kind` 経由でスワップする経路は
残してある（運用上は parse-time 発行でカバーされる）。
