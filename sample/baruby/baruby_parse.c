...
void
baruby_update_sp_bodies_from_cc(void)
{
    for (struct pg_call_link *l = all_pg_call_nodes_head; l; l = l->next) {
        if (l->cc->serial == 0 || l->cc->body == NULL) continue;
        // sp_body is excluded from HORG (baruby_gen.rb hash_call kind=horg)
...
callsite_resolve(void)
{
    ...
    while (cs) {
        ...
        // sp_body is excluded from HORG (see baruby_gen.rb), so the
...
transduce(struct transduce_context *tc, pm_node_t *node, int indent) {
    ...
    switch (node->type) {
      ...
      case PM_AND_NODE: {
          ...
          // Simplified: `&&` returns true/false (not the operand), which
          // is sufficient for `if` / `while` predicates.
          return ALLOC_node_if(TRANSDUCE(n->left),
                  ALLOC_node_if(TRANSDUCE(n->right), ALLOC_node_true(), ALLOC_node_false()),
                  ALLOC_node_false());
      }
      ...
      case PM_ARRAY_NODE: {
          ...
          // Iter 36 Perf 1 (ported from baruby_precise): direct 1-shot
          // construction for small N.  Saves alloc+wb overhead vs
          // ary_new + ary_push chain.  Effective especially in AOT mode.
      ...
      case PM_CALL_NODE: {
          ...
          // the corresponding type-dispatching node.  Keeps the surface
      ...
      case PM_DEF_NODE: {
          ...
          // body_locals MUST be max_cnt (parser's high-water for arg /
          // nested-call slots), not just n->locals.size.  Smaller values
          // cause the callee's lset chain (writing fp[arg_idx + i]) to
          // alias with caller's @child snapshots or arg-eval slots when
          // a >3-arg call appears inside an @child body.  See
          // baruby_precise/baruby_parse.c for the matching fix + rationale.
      ...
      case PM_OR_NODE: {
          ...
          // Like `&&` above: result is normalised to true/false.
          return ALLOC_node_if(TRANSDUCE(n->left),
                  ALLOC_node_true(),
                  ALLOC_node_if(TRANSDUCE(n->right), ALLOC_node_true(), ALLOC_node_false()));
      }
      ...
      case PM_STRING_NODE: {
          ...
          // Copy into a stable buffer — pm_string_t may free / move with
          // the parser, while the NODE outlives parse.  malloc routes
          // through libgc so the literal is reclaimed if the AST dies.
...
const char *baruby_current_source_file = NULL;

NODE *
PARSE(int argc, char *argv[])
{
    const char *fname = parse_option(argc, argv);
    baruby_current_source_file = fname;
