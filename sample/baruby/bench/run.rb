...
BARUBY    = File.expand_path("../baruby", BENCH_DIR)
...
puts "%-24s %10s %10s %10s %10s" % ["bench", "best(s)", "med(s)", "alloc_MB", "GCs"]
...
benches.each do |path|
  ...
  opts[:repeats].times do
    ...
    times << real if real
    if (m = out.match(/__GC_STATS__ alloc_bytes=(\d+) heap_bytes=\d+ gc_count=(\d+)/))
  ...
  if times.empty?
    ...
    sorted = times.sort
    puts "%-24s %10.3f %10.3f %10.1f %10d" %
         [name, sorted.first, sorted[sorted.size / 2], alloc_mb || 0.0, gcs || 0]
  end
end
