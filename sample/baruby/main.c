...
main(int argc, char *argv[])
{
    ...
    if (!OPTION.compile_only) {
        ...
        if (getenv("BARUBY_GC_STATS")) {
            // Boehm libgc counters.  GC_get_total_bytes is the cumulative
            // allocation total (never decreases); GC_get_heap_size is the
            // current heap size; GC_get_gc_no returns the collection count.
            size_t alloc_bytes = GC_get_total_bytes();
            size_t heap_bytes  = GC_get_heap_size();
            size_t gc_count    = GC_get_gc_no();
