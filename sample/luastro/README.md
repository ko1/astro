...
| benchmark   | plain   | AOT-1st | AOT-cached | lua5.4  | luajit  | vs lua5.4   |
|-------------|--------:|--------:|--------:|--------:|--------:|------------:|
...
- **plain** — pure interpreter (no `code_store/`).
- **AOT-1st** — `-c`: bake + run in one process; `code_store/`
  cleared per iteration so timing **includes** gcc compile time.
- **AOT-cached** — bake once in `setup` (not timed), then time
