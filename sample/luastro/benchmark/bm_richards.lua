-- AWFY richards (operating-system simulator: 5 task types, message-passing
-- packet flow, 4 priority levels).  Heavy method dispatch through metatable
-- chains; verifies the IC tier on luastro.
-- Source: https://github.com/smarr/are-we-fast-yet (benchmarks/Lua/richards.lua)
-- Stripped of `require'benchmark'`; replaced bit32-fallback `band/bxor` with
-- native Lua 5.3+ `&` / `~` operators.
...
local r = setmetatable({}, {__index = richards})
local ITERS = 50
local result
for _ = 1, ITERS do result = r:benchmark() end
print(result)  -- lua5.4: true
