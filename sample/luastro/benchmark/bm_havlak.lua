-- AWFY havlak (loop finder; SOM benchmark suite).  Heavy use of
-- IdentityDictionary on BasicBlock objects + multi-level metatable
-- chains.  Stresses the table integer-key array/hash split.
-- Source: github.com/smarr/are-we-fast-yet (benchmarks/Lua/havlak.lua)
-- som collections inlined (Vector/Set/IdentitySet/Dictionary/IdentityDictionary).
...
local d = setmetatable({}, {__index = havlak})
local ITERS = 1
local result
for _ = 1, ITERS do result = d:inner_benchmark_loop(1) end
