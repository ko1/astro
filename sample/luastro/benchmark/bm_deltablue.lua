-- AWFY deltablue (constraint satisfaction solver: object graph, dependency
-- propagation, plan execution).  Stresses metatable inheritance + dict ops.
-- Source: https://github.com/smarr/are-we-fast-yet (benchmarks/Lua/deltablue.lua)
-- som collections inlined.

-- som.lua collections inlined (Vector / Set / IdentitySet / Dictionary /
-- IdentityDictionary / Random).
-- Source: github.com/smarr/are-we-fast-yet/blob/master/benchmarks/Lua/som.lua
-- Cleaned for luastro: stripped `_VERSION` / `bit32` fallback (Lua 5.3+ only,
-- native `&` / `~` / `>>`); stripped `pcall(require, 'table.new')` LuaJIT
-- extension (only the fallback `alloc_array` is kept).
...
function Vector:at (idx)
    if self.storage == nil or idx > self.storage.n then return nil end
    return self.storage[idx]
end

function Vector:at_put (idx, val)
    if self.storage == nil then
        self.storage = alloc_array(max(idx, 10))
    elseif idx > self.storage.n then
        local new_n = self.storage.n
        while idx > new_n do new_n = new_n * 2 end
        local new_storage = alloc_array(new_n)
        for i = 1, self.storage.n do new_storage[i] = self.storage[i] end
        self.storage = new_storage
    end
    self.storage[idx] = val
    if self.last_idx < idx + 1 then self.last_idx = idx + 1 end
end

function Vector:append (elem)
    if self.storage == nil then
        self.storage = alloc_array(10)
...
function Vector:is_empty () return self.last_idx == self.first_idx end

function Vector:each (fn)
    for i = self.first_idx, self.last_idx - 1 do fn(self.storage[i]) end
end

function Vector:has_some (fn)
    for i = self.first_idx, self.last_idx - 1 do
        if fn(self.storage[i]) then return true end
    end
    return false
end
...
function Vector:remove (obj)
    if self.storage == nil or self:is_empty() then return false end
    local new_array = alloc_array(self:capacity())
    local new_last = 1
    local found = false
    self:each(function (it)
        if it == obj then found = true
        else new_array[new_last] = it; new_last = new_last + 1 end
    end)
    self.storage = new_array; self.last_idx = new_last; self.first_idx = 1
    return found
end

function Vector:remove_all ()
    self.first_idx = 1; self.last_idx = 1
    if self.storage ~= nil then self.storage = alloc_array(self:capacity()) end
end

function Vector:size () return self.last_idx - self.first_idx end

function Vector:capacity ()
    if self.storage == nil then return 0 end
    return self.storage.n
end

function Vector:sort (fn)
    if self:size() > 0 then self:sort_range(self.first_idx, self.last_idx - 1, fn) end
end
...
function Vector:sort_range (i, j, fn)
    ...
    local di, dj = storage[i], storage[j]
    if not fn(di, dj) then
        storage[i], storage[j] = dj, di
        di, dj = dj, di
    end
    if n > 2 then
        local ij = floor((i + j) / 2)
        local dij = storage[ij]
        if fn(di, dij) then
            if not fn(dij, dj) then
        ...
        if n > 3 then
            local k, l = i, j - 1
            while true do
                while k <= l and fn(dij, storage[l]) do l = l - 1 end
                k = k + 1
                while k <= l and fn(storage[k], dij) do k = k + 1 end
...
function Set.new (size)
    return setmetatable({items = Vector.new(size or 10)}, Set)
end

function Set:size () return self.items:size() end
function Set:each (fn) self.items:each(fn) end
function Set:has_some (fn) return self.items:has_some(fn) end
function Set:get_one (fn) return self.items:get_one(fn) end
function Set:remove_all () self.items:remove_all() end

function Set:add (obj)
    if not self:contains(obj) then self.items:append(obj) end
end

function Set:collect (fn)
    local coll = Vector.new()
    self:each(function (it) coll:append(fn(it)) end)
    return coll
end
...
local IdentitySet = setmetatable({}, {__index = Set})
IdentitySet.__index = IdentitySet

function IdentitySet.new (size)
    local obj = Set.new(size)
    return setmetatable(obj, IdentitySet)
end
...
function Entry.new (hash, key, value, next)
    return setmetatable({hash=hash, key=key, value=value, next=next}, Entry)
end
...
function Dictionary.new (size)
    return setmetatable({buckets = alloc_array(size or 16), size = 0}, Dictionary)
end

function Dictionary:hash (key)
    if not key then return 0 end
    local h = key:custom_hash()
    return h ~ (h >> 16)
end

function Dictionary:is_empty () return self.size == 0 end

function Dictionary:get_bucket_idx (hash)
    return ((self.buckets.n - 1) & hash) + 1
end
...
function Dictionary:at (key)
    ...
    while e do
        if e:match(hash, key) then return e.value end
        e = e.next
    end
    return nil
end
...
function Dictionary:contains_key (key)
    ...
    local e = self:get_bucket(hash)
    while e do
        if e:match(hash, key) then return true end
        e = e.next
    end
    return false
end
...
function Dictionary:at_put (key, value)
    ...
    if self.size > self.buckets.n then self:resize() end
end
...
function Dictionary:transfer_entries (old_storage)
    ...
    for i = 1, old_storage.n do
        ...
        if current then
            old_storage[i] = nil
            if not current.next then
                local h = (current.hash & (buckets.n - 1)) + 1
                buckets[h] = current
            else
...
function Dictionary:split_bucket (old_storage, i, head)
    local lo_head, lo_tail
    local hi_head, hi_tail
    local current = head
    while current do
        if (current.hash & old_storage.n) == 0 then
            if not lo_tail then lo_head = current
            else lo_tail.next = current end
            lo_tail = current
        else
            if not hi_tail then hi_head = current
            else hi_tail.next = current end
            hi_tail = current
        end
        current = current.next
    end
    if lo_tail then lo_tail.next = nil; self.buckets[i] = lo_head end
    if hi_tail then hi_tail.next = nil; self.buckets[i + old_storage.n] = hi_head end
end
...
function Dictionary:keys ()
    ...
    for i = 1, buckets.n do
        ...
        while current do keys:append(current.key); current = current.next end
    end
    return keys
end
...
function Dictionary:values ()
    ...
    for i = 1, buckets.n do
        local current = buckets[i]
        while current do vals:append(current.value); current = current.next end
    end
    return vals
end

-- ============================================================ IdEntry / IdentityDictionary

local IdEntry = setmetatable({}, {__index = Entry})
IdEntry.__index = IdEntry

function IdEntry.new (hash, key, value, next)
    local obj = Entry.new(hash, key, value, next)
    return setmetatable(obj, IdEntry)
end
...
local IdentityDictionary = setmetatable({}, {__index = Dictionary})
IdentityDictionary.__index = IdentityDictionary

function IdentityDictionary.new (size)
    local obj = Dictionary.new(size)
    return setmetatable(obj, IdentityDictionary)
end
...
local d = setmetatable({}, {__index = deltablue})
local ITERS = 500
local result
for _ = 1, ITERS do result = d:inner_benchmark_loop(50) end
print(result)  -- expects: true
