-- AWFY json (parses a 27 KiB JSON document — string-heavy + table-heavy).
-- Source: https://github.com/smarr/are-we-fast-yet (benchmarks/Lua/json.lua)
-- Vector and HashIndexTable inlined from som.lua / hashindextable-53.lua;
-- require'benchmark' inheritance stripped.

-- Vector class (subset of github.com/smarr/are-we-fast-yet/blob/master/benchmarks/Lua/som.lua).
-- Inlined for self-containment (luastro has no `require`); LuaJIT-only `table.new`
-- branch dropped, fallback used.
...
local j = setmetatable({}, {__index = json})
local ITERS = 100
local result
for _ = 1, ITERS do result = j:benchmark() end
print(j:verify_result(result))  -- expects: true
