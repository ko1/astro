...
| ~~**Pinned +0.0 cell**~~ | **Done.**  A single shared `LuaHeapDouble` represents `0.0` instead of `calloc`-ing a fresh cell on every accumulator-hits-zero / loop-init.  Mandelbrot AOT-cached: 108 → 88 ms (-19%). |
...
| ~~**Lazy array-part promotion for sparse-start integer keys**~~ | **Done.**  `lua_table_seti` now accepts ≤ 2× arr_cap + 4 as "modestly close" and routes to the array part instead of dropping every entry into the open-addressing hash.  This unblocks the common `for i=2,N do t[i] = ... end` pattern (sieve, etc.) that previously bypassed the array entirely because index 1 was never set.  Sieve AOT-cached: 12 → 6 ms — **tied with lua5.4**. |
...
| ~~**Make every dispatched NODE reachable through dlsym**~~ | **Done.**  Initially solved with a file-rewrite hack (`luastro_export_sd_wrappers`: rename every `SD_<hash>` → `SD_<hash>_INL` and append weak extern wrappers).  Replaced with proper entry registration via `astro_cs_compile` for the chunk root, every `CR.entries[i].body` (closure body), and every `LUASTRO_NODE_ARR[i]` (variadic operand) — the framework natively emits each entry as public-extern with its subtree as `static inline`, so dlsym finds every NODE that the engine actually dispatches at runtime.  Original wrapper hack ~150 lines deleted; entry registration is ~20 lines and matches the framework's intended use.  Mandelbrot AOT-cached: 76 → 65 ms (-14%) preserved across the refactor. |
| ~~**Variadic-child SD baking (`LUASTRO_NODE_ARR` walk)**~~ | **Done.**  `@noinline` nodes (`node_local_decl`, multi-arg calls, table constructors) hide their children inside `LUASTRO_NODE_ARR` (the parser's side array), and ASTroGen's specializer walks only typed `NODE *` operands so those children stayed unbaked.  `luastro_specialize_all` now also calls `astro_cs_compile` on each entry of `LUASTRO_NODE_ARR` so every node ends up addressable by `dlsym`.  cs-hit rate goes from ~80% → ~95% on mandelbrot.  Mandelbrot AOT-cached: 65 → 60 ms (-8%). |
| ~~**`node_local_decl_one` for `local x = expr`**~~ | **Done.**  The hot inner-loop pattern `local xn = x*x - y*y + x0` was being routed through `@noinline` `node_local_decl`'s side-array path even after the `nlhs==1 && nrhs==1` runtime fast path landed.  Added a dedicated parser-emitted node `node_local_decl_one(slot, NODE *rhs)` with `@always_inline`; ASTroGen now recurses into the typed `rhs` operand and bakes an SD that inlines the rhs evaluation directly.  `cs hit` jumps from 99/106 → 104/106; perf-attribution `DISPATCH_node_local_decl` drops from 9% → 1%.  Mandelbrot AOT-cached: 60 → 51 ms (-15%). |
| ~~**Inlined `luav_unbox_double`**~~ | **Done.**  Heap-boxed double decode is now `static inline __attribute__((always_inline))` in `context.h` (was an out-of-line call); the heap-fallback path in `luav_to_double` becomes a direct struct-field load.  Mandelbrot AOT-cached: 51 → 47 ms (-8%). |
| ~~**Stop re-interning string operands on every dispatch**~~ | **Done.**  `lua_gen.rb`'s specializer was emitting `lua_str_intern("x")` literally into SD bodies — the comment said "Re-intern by C string at SD load time so the SD_ source can be cached on disk and reloaded across runs", but that meant **every dispatch re-ran the intern** on the same C string.  perf showed `lua_str_intern_n` at 21.5% of nbody AOT-cached cycles.  The parser already interns the string and stores the `LuaString *` on the AST node, so the SD just reads `n->u.<kind>.<field>` — fresh-per-run because the AST is rebuilt by the parser before any SD runs.  Nbody AOT-cached: 33 → 22 ms (-33%). |
| ~~**Shape-token inline cache on `node_field_get`**~~ | **Done.**  `node_field_get` carries an inline `struct LuaFieldIC { uint32_t shape; uint32_t pos }` (`@ref` operand) that caches the table's `hash_cap` at last successful resolve plus the slot offset for the field key.  Hot path: if `t->hash_cap == ic->shape` and `e->key == LUAV_STR(field)` then return `e->val` — no hash, no probe.  Works because two tables built from the same factory (e.g. nbody's `body(...)` returning `{x=, y=, z=, vx=, ...}`) have identical `hash_cap` and identical slot positions for the same interned-string key.  Generalizes to any "stable shape" workload, not just nbody.  Nbody AOT-cached: 22 → 17 ms (-23%). |
...
| ~~**Type-speculating SDs**~~ | **Done.**  PGC type-spec landed (`91f8544`).  Each base arith / compare body (`node_int_add` / `_sub` / `_mul` / `node_lt` / `node_le`) calls `swap_dispatcher` on first observed int+int (or flt+flt) hit, promoting to a strict variant (`_ii` / `_ff`) whose SD body has the type-mono path inline and the cold case (mixed numeric, metamethod fallback) extracted to a `noinline cold` helper.  `@canonical=node_int_<op>` so HORG matches between the parser-allocated base kind and the post-swap kind, letting `(HORG, file, line) → HOPT` resolve at cached load time.  Bench (PGC-cached vs AOT-cached): nbody -19%, fannkuch -13%, mandelbrot -6%; richards / deltablue / json ≈ tie. |
| ~~**Per-thread `malloc` arena overhead in `-p` bake**~~ | **Done.**  `luastro_run_on_big_stack` spawns a worker pthread for stack safety; glibc gives that pthread its own arena, mmap'd lazily and committed in 4 KiB chunks via `mprotect(PROT_READ|PROT_WRITE)`.  bm_storage saw 58 K mprotect calls = ~220 ms of pure kernel time per run.  `mallopt(M_ARENA_MAX, 1)` in `main` routes the worker's mallocs through the main thread's `brk` arena: storage 0.42 s → 0.30 s (-29%), binarytrees -32%. |
...
- **Profile-driven loop fusion** — detect type-stable accumulator
  patterns at runtime and bake fused-body SDs from profile.  Earlier
  attempt (a parser-pattern-matched `node_numfor_int_sum` that ate
  exactly `for i=...do sum = sum + i end`) was removed because the
  pattern was too specific and acted as a benchmark cheat: gcc's
  scalar-evolution pass replaced the fused C loop with the closed-form
  `N*(N+1)/2`, making the loop benchmark measure gcc's optimiser
  rather than the interpreter.  Any future fusion needs to be
  workload-driven, not syntactic.
