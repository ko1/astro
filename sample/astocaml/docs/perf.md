...
| ベンチ | 入力 | 結果 | 時間 |
|--|--|--|--|
| ack    | ack(3, 9)         | 4093    | 0.081 s |
| fib    | fib(35)           | 9227465 | 0.113 s |
| nqueens| 10-queens × 3     | 724     | 0.253 s |
| sieve  | sum prime ≤8000 ×8 | 3738566 | 0.279 s |
| tak    | tak(24,16,8) ×5   | 9       | 0.054 s |

(`make pgo` 一発で astocaml binary + SD `.so` 両方に PGO を適用。 起動時 `pgo-sd/` を自動検出して `-fprofile-use` を SD に注入)
