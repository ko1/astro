...
- cons の bump allocator (4MB chunk) はそのまま残す。chunk 自体が `GC_malloc` 経由で確保されるが Boehm からは 1 個の object として扱われ、内部の cons cell 単位の GC はされない (chunk 内に 1 個でも live なら chunk 全体が live)。bump allocator の速度を保つトレードオフ
- **効果**: 1M 回の `let _ = ref [1;2;..;10] in loop (n-1)` で peak RSS 33 MB に収まる (GC なしなら数百 MB に膨張)
- ベンチ影響: ack/fib/tak は誤差レベル、sieve のみ 17% 程度 regress (GC pause + 大量 cons 領域の scan)
