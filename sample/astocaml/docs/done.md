...
`main.c` に `#include <gc.h>` + `#define malloc GC_malloc` / `calloc` / `realloc` / `free` (no-op) のローカル override で全 runtime allocation を GC 経由に。`main()` で `GC_INIT()`、Makefile に `-lgc`。

- 影響範囲: oobj (closure / variant / record / ref / float / string / array / cons pool / ...)、oframe (alloca しない non-leaf)、CTX、globals table、type checker 内部構造、partial_state、closures' captured args
- AST node (`node.c` の `node_allocate`) は plain malloc のまま — プログラム終了まで生きる前提なので GC 管理不要。Boehm の conservative scan が AST NODE * を見ても opaque pointer として無視されるだけ (NODE storage は free されないので safe)
- cons cell も per-cell `GC_malloc` で確保 (bump allocator 撤去)。Boehm の small-object freelist が cons cell を再利用するので、bump 版より **逆に速い** (nqueens 0.355 → 0.275, sieve 0.434 → 0.322)。cons が個別に GC されるので long-run でも cons 領域が flat
- **効果**: 1M iter の `let _ = ref [1;2;..;10] in loop (n-1)` で peak RSS 36 MB (GC なしなら数百 MB)
- ベンチ影響: 全般で誤差レベル or 改善 (cons-heavy ベンチは特に)

### PGO (Profile-Guided Optimization)

gcc 標準の PGO を astocaml binary と SD `.so` 両方に適用。`make pgo` で全自動:

1. `pgo-bin-gen`: astocaml を `-fprofile-generate=pgo/` で再ビルド
2. `pgo-bin-train`: bench/*.ml を流して main binary の hot path 統計を収集
3. `pgo-bin`: `-fprofile-use=pgo/` で再ビルド
4. `pgo-sd-gen`: PGO化された astocaml で再度 bench を流すが、今度は `ASTRO_EXTRA_CFLAGS=-fprofile-generate=pgo-sd/` で SD `.so` 側を instrument
5. `pgo-sd`: `-fprofile-use=pgo-sd/` で SD を再ビルド + warmup

`maybe_aot_compile` が `pgo-sd/` ディレクトリの存在を起動時にチェックし、あれば自動で `-fprofile-use` を `ASTRO_EXTRA_CFLAGS` に注入。**ユーザは普通に `./astocaml -c <bench>` するだけで PGO 効果を得られる**。

`ASTRO_NO_PGO=1` で無効化可能。
...
| ack | 0.090 | **0.081** | 11% |
| fib | 0.146 | **0.113** | 22% |
| nqueens | 0.275 | **0.253** | 8% |
| sieve | 0.322 | **0.279** | 13% |
| tak | 0.075 | **0.054** | **31%** |

特に **tak 31%, fib 22%** の大幅改善。 ocamlopt との差は fib **2.9×**, tak **3.6×** まで縮まった (これまでで最高)。

注意: method_call.ml は `-fprofile-generate` 下で segfault するため training set から除外。理由は libgc のスタック scan + gcov の TLS instrumentation の干渉と思われる。
