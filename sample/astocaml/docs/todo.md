...
- **Closure leaf 検出 + alloca フレーム** (内部に新しい closure を作らないクロージャはスタック上に確保可) — TCO と組み合わせて完全な C ループ並みに
...
- **method send IC** (object#method の dispatch を inline cache 化)
...
| ack    | 4093    | 0.80 s |
| fib    | 9227465 | 1.44 s |
| nqueens| 724 (×3) | 1.24 s |
| sieve  | 3738566 (×8) | 1.19 s |
| tak    | 9 (×5) | 0.73 s |

`--compile` (AOT) でさらに小幅改善。

将来計画:

- closure leaf alloca + 環境キャッシュで深い tail loop を C ループ並みに
