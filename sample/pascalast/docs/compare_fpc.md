...
-----                    ------  -------  -------      ---
ackermann                  1.38     0.17     0.05     0.19
collatz                    1.32     0.15     0.12     0.05  *
fib                        0.75     0.13     0.07     0.12
gcd                        0.89     0.23     0.11     0.13
heron                      1.28     0.14     0.07     0.07  ≈
leibniz_pi                 1.07     0.12     0.11     0.07  *
mandelbrot_int             1.45     0.05     0.04     0.03  *
...
oop_shapes                 0.96     0.10     0.05     0.25
quicksort                  0.98     0.14     0.09     0.18
sieve                      1.19     0.13     0.06     0.17
tarai                      1.42     0.18     0.10     0.19
varparam_swap              1.10     0.07     0.02     0.06
```

`*` AOT beats fpc -O3, `≈` AOT matches fpc -O3.

**Closest to fpc -O3** (within 1.5×):
- gcd 1.2× (≈ measurement noise)
- mandelbrot_int 1.3× — actually faster

**Within 2.1×** (call-heavy benches that still pay protocol overhead):
- fib 1.7×, tarai 1.9×, quicksort 2.0×

**Biggest remaining gaps** (3-5×):
- oop_shapes 5.0× — pure virtual-dispatch protocol cost
- ackermann 3.8× — pure call protocol; body is one int op
- sieve 2.8× — array index + write hot loop, fpc auto-vectorises
...
**pascalast AOT beats fpc -O3 on 5 of 15 benches** (+ matches on 2):

| bench           | fpc -O3 | AOT    | speedup |
|---|---|---|---|
| nested_loops    | 0.08 s  | 0.00 s | ≥ 80 × |
| collatz         | 0.12 s  | 0.05 s | 2.4 ×  |
| leibniz_pi      | 0.11 s  | 0.07 s | 1.6 ×  |
| heron           | 0.07 s  | 0.07 s | 1.0 ×  |
| mandelbrot_int  | 0.04 s  | 0.04 s | 1.0 ×  |
...
|---|---|---|---|
| ackermann       | 0.05 s  | 0.24 s | 4.8 ×  |
| sieve           | 0.06 s  | 0.20 s | 3.3 ×  |
| matmul_2d       | 0.06 s  | 0.18 s | 3.0 ×  |
| matmul          | 0.02 s  | 0.05 s | 2.5 ×  |
| tarai           | 0.10 s  | 0.23 s | 2.3 ×  |
| fib             | 0.07 s  | 0.16 s | 2.3 ×  |
| quicksort       | 0.09 s  | 0.20 s | 2.2 ×  |
| varparam_swap   | 0.02 s  | 0.07 s | 3.5 ×  |
| gcd             | 0.11 s  | 0.23 s | 2.1 ×  |

Even after rounds 8 + 8' (baked pcall, vcall IC, fp threading) the
call protocol still has real cost — c->sp updates, slot zero-fill,
and the `pascal_call_baked` invocation overhead.  A bare native
`call` instruction is hard to beat with a tree-walker.  But fp
threading (round 8') closed a meaningful chunk of this gap by
removing c->fp memory traffic from every dispatch.
