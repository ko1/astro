...
| **Visibility enforcement for class fields** — `private` / `protected` / `public` / `published` sections inside `class … end` are tracked per field; `check_access` is called at every field load / store site (`obj.field`, `Self.field` via bare-name auto-resolve, `(p as T).field`).  Private fields are accessible only from methods of the *declaring* class; protected from the declaring class and any descendant.  Plain (non-class) record fields skip the check (`decl_class = -1`).  Method / property visibility enforcement is deferred — fields are the load-bearing case | `test/49_visibility.pas` |
