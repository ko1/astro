...
- `ALLOC_<name>` — node を割り当て、末尾で `OPTIMIZE(_n)` を呼ぶ
- `HASH_<name>` — Merkle ハッシュ計算 (Horg)
- `DUMP_<name>` — pretty print
- `SPECIALIZE_<name>` — 部分評価された C を出力 (`astro_cs_compile` から呼ばれる)
...
`n->head.dispatcher` を渡しているため、SD 化されると間接呼び出しが
gcc によって直接呼び出しに畳まれる。

## AOT / PG 特化 (SD bake)

jstro は ASTro の dispatcher / evaluator 分離をフルに活かす。`-c`
(AOT) または `-p` (PG) を渡すと:

1. **specialize**: parse 済み AST を `astro_cs_compile` でそれぞれ
   `code_store/c/SD_<Horg>.c` (AOT) / `code_store/c/PGSD_<Hopt>.c`
   (PG) に出力。各 SD は ASTroGen が生成した `SPECIALIZE_<name>`
   が「自分自身の dispatcher を inline 展開した C 関数」を吐く。
   typed-operand を持たない variadic node (`call`/`object_lit` 等)
   の子は `JSTRO_NODE_ARR` を直接走査して個別に bake する。
2. **build**: `astro_cs_build` が `code_store/Makefile` を吐いて
   `gcc -O3 -fPIC -fno-plt -march=native` で `all.so` にリンク。
3. **wrapper export**: `jstro_export_sd_wrappers` が各 SD ファイルを
   後処理し、`SD_<h>` を全部 `SD_<h>_INL` にリネームしたうえで末尾に
   `__attribute__((weak)) RESULT SD_<h>(CTX*c, NODE*n, JsValue*frame) {
   return SD_<h>_INL(c, n, frame); }` という weak extern wrapper を
   append する。これで `dlsym(SD_<h>)` が必ず見つかり、`astro_cs_load`
   が「ルート以外の SD」も解決できるようになる。
4. **reload + repatch**: `astro_cs_reload` で `dlopen` をやり直し、
   登録された全ノード (root + `code_repo` の関数本体 +
   `JSTRO_NODE_ARR`) について `astro_cs_load` を呼び直して
   `head.dispatcher` を更新する。
5. **run on big stack**: 深い AOT-inline 鎖が C スタックを食うので、
   実行は `pthread_create` で 4 GiB virtual stack の worker thread に
   切り出す。

`-c` は (1)-(5) を 1 プロセスで、`--aot-compile` は (1)-(4) のみ。
キャッシュ済み `code_store/all.so` がある状態で `./jstro file.js` を
起動すると、`astro_cs_init` が起動時に `dlopen` し、parse 中に各
`OPTIMIZE(_n)` が `astro_cs_load` を経由して dispatcher を patch する。

PG (`-p`) は今のところ AOT と等価 (jstro は profile-driven な
`swap_dispatcher` を持っていないので Hopt == Horg)。整数/double 専用
ノードを足せば PG の出番が出てくる。
