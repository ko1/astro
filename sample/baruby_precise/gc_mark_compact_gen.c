...
#define TENURED_BYTES  ((size_t)512u << 20)    // 512 MiB (single region)
...
typedef struct GCHeader {
    ...
    bool     marked;    // tenured object is reachable in the current major
    // 5 bytes padding to keep payload 8-aligned
} GCHeader;
...
const char *baruby_gc_backend_name = "mark_compact_gen";
...
major_gc(VALUE *sp_top)
{
    ...
    // (3) Update outgoing pointers in each live object (in place).
    {
        ...
        while (p < tenured_top) {
            GCHeader *h = (GCHeader *)p;
            size_t total = sizeof(GCHeader) + ALIGN8(h->size);
            if (h->marked) update_pointers_major(h);
            p += total;
        }
    }
