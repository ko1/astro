#!/bin/bash
# Run each adapted PolyBench kernel under castro AOT, gcc -O0, gcc -O3.
# Reports median wall-clock + correctness check.
set -eu
...
# castro environment.  CCACHE_DISABLE=1 because the parent sandbox often
# blocks the default ccache cache dir at $HOME/.cache/ccache (read-only
# in the sandbox), which makes `make` exit 512 silently and falls back
# to the interpreter without an obvious error.
export CCACHE_DIR=$ROOT/tmp/ccache
export CCACHE_DISABLE=1
mkdir -p "$CCACHE_DIR"

# Pick a list of kernels (or honour positional args).
if [[ $# -gt 0 ]]; then
    KERNELS="$@"
else
...
# Pre-build gcc references and pre-compile castro AOT for every kernel.
for k in $KERNELS; do
    src=$KERNELS_DIR/$k.c
    ...
    # `--compile-all` always runs the program after the AOT build, so
    # its exit code is the program's checksum, not a build status.
    # Check for the actual `all.so` artefact instead.
    "$CASTRO" --compile-all "$src" >/dev/null 2>&1 || true
    if [[ ! -f code_store/all.so ]]; then
        echo "AOT build failed for $k (no code_store/all.so)" >&2
        continue
    fi
    # Snapshot the code_store dir so the timing loop below can restore
    # it cleanly per kernel.  `cp -r SRC DST` semantics: if DST exists,
    # the result is DST/SRC (= nested!) — so we rm DST first.
...
for k in $KERNELS; do
    ...
    rm -rf code_store
    [[ -d "$BIN_DIR/${k}_code_store" ]] && cp -r "$BIN_DIR/${k}_code_store" code_store
