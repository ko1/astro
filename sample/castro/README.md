...
- 主な高速化:
  - **コンパイル時に呼び先決定**: `(call NAME ...)` を `(call FUNC_IDX ...)` に変更し、parse 時に解決した index で `c->func_set[idx]` を直接引く。inline cache (callcache / func_addr_cache) + serial 検証 + dispatcher snapshot + 名前ルックアップを全部削除
