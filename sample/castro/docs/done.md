...
| 機構 | 効果 |
|---|---|
| **コンパイル時に呼び先決定** | parse.rb が各 `function_definition` に index を振り、`(call FUNC_IDX nargs arg_idx)` を発行。runtime は `c->func_set[idx]` を直接引く。strcmp / 線形探索 / serial 検証 / inline cache 全廃 |
...
| **`-rdynamic` ビルド** | code_store の SD が host 側ヘルパー (`castro_invoke_jmp` など) を参照できる (これがないと AOT が黙って interp に fallback) |

### ベンチ結果 (gcc -O0 と -O3 比較)

`bench.rb` は parse.rb の起動 (~110ms) を除いた純粋な実行時間で比較。
11 件、`BENCH_RUNS=15` median。

| bench              | interp | AOT first | AOT cached | gcc-O0 | gcc-O3 | castro/-O3 |
|--------------------|-------:|----------:|-----------:|-------:|-------:|---:|
| fib_big (fib 35)   |    427 |        69 |     **48** |     47 |     16 | 3.0× |
| fib_d              |     15 |        23 |      **4** |      3 |      1 | 4× |
| tak (18,12,6)      |      4 |        21 |      **2** |      0 |      0 | -- |
| ackermann (3,8)    |    119 |        31 |     **12** |      8 |      1 | 12× |
| loop_sum           |    263 |        21 |   🟢 **2** | 🔴  9  |      0 | 2× |
| mandelbrot_count   |     25 |        23 |      **3** |      2 |      1 | 3× |
| sieve              |    167 |        28 |      **8** |      8 |      2 | 4× |
| nqueens            |    582 |        89 |     **69** |     31 |     14 | 4.9× |
| quicksort          |   3110 |        96 |  🟢 **84** | 🔴 92  |     23 | 3.6× |
| crc32              |    831 |        33 |  🟢 **14** |     46 | 🔴 18  | **0.78×** |
| matmul             |     58 |        27 |      **8** |      2 |      0 | -- |
