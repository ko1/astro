...
| 機構 | 効果 |
|---|---|
| **コンパイル時に呼び先決定** | parse.rb が各 `function_definition` に index を振り、`(call FUNC_IDX nargs arg_idx)` を発行。runtime は `c->func_bodies[idx]` を直接引く。strcmp / 線形探索 / serial 検証 / inline cache 全廃 |
