...
- AOT cached が gcc -O0 を上回るのは loop_sum (4 vs 9、2.25× 速)
- 残り 5 ベンチは gcc -O0 の 1.3〜1.5× 圏内 (fib_big 1.39×、ackermann 1.38×)
- 主要マイルストーン:
  - **改善前 (return-lift / `-rdynamic` も無し)**: AOT cached fib_big = 1442ms
  - tail-return リフト + `@ref` callcache + `-rdynamic` 投入後: 112ms (約 13×)
  - **本コミット (call IR を name-based から index-based に置き換え)**: 64ms (さらに 1.75×)
