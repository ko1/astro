...
static VALUE ab_float_add(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return AB_NUM_WRAP(rb_funcall(SELF_F, rb_intern("+"), 1, ARG_F));
}
...
static VALUE ab_float_neg(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return abruby_float_new_wrap(rb_float_new(-RFLOAT_VALUE(SELF_F)));
}

static VALUE ab_float_lt(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return RTEST(rb_funcall(SELF_F, rb_intern("<"), 1, ARG_F)) ? Qtrue : Qfalse;
}
...
static VALUE ab_float_neq(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return RTEST(rb_funcall(SELF_F, rb_intern("=="), 1, ARG_F)) ? Qfalse : Qtrue;
}

static VALUE ab_float_inspect(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return abruby_str_new(rb_funcall(SELF_F, rb_intern("to_s"), 0));
}
...
static VALUE ab_float_to_i(CTX *c, VALUE self, unsigned int argc, VALUE *argv) {
    return AB_NUM_WRAP(rb_funcall(SELF_F, rb_intern("to_i"), 0));
}
...
Init_abruby_float(void)
{
    ...
    abruby_class_set_const(ab_float_class, "INFINITY", abruby_float_new_wrap(rb_float_new(HUGE_VAL)));
    abruby_class_set_const(ab_float_class, "NAN", abruby_float_new_wrap(rb_float_new(nan(""))));
