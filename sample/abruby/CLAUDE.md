...
- **`method_cache_fill`** — mc にキャッシュ内容（klass, method, serial, body, dispatcher）を書き込む。
...
struct abruby_class {
    ...
    struct abruby_method methods[64]; // メソッドテーブル (AST or CFUNC)
    unsigned int method_cnt;
    ...
};
```

- ビルトインクラス: Object, Class, Integer, Float, String, Symbol, Array, Hash, Range, Regexp, Rational, Complex, TrueClass, FalseClass, NilClass
...
- VALUE スタック (CTX 内) は machine の T_DATA ラッパー経由で `rb_gc_mark_locations` でマーク
...
- test_global_variables (9), test_multi_assign (15)
- test_gc_pressure (20)
