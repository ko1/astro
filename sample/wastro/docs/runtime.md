...
emits a fixed-shape struct per node, so wastro provides arity-specific
variants:

| Node                       | Arity  | Notes                                         |
|----------------------------|--------|-----------------------------------------------|
| `node_call_0..4`           | 0–4    | direct call to defined function               |
| `node_call_indirect_0..4`  | 0–4    | `call_indirect` via `WASTRO_TABLE[idx]`       |
| `node_host_call_0..3`      | 0–3    | call into a host-imported function            |

The parser (`parse_call_expr` in `main.c`) selects by argument count.
Anything beyond arity 4 is rejected; raise the limit by adding more
variants — most real-world modules stay within this range.
