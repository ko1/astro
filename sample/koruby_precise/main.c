...
// Forward decl from parse.c — walks the all_pg_call_nodes list
...
void koruby_update_sp_bodies_from_cc(void);

NODE *PARSE(int argc, char *argv[]);
extern const char *koruby_current_source_file;
...
{
    DEFINE_FUNC(c, "p", korb_p, 1);
    DEFINE_FUNC(c, "puts", korb_puts, 1);
    DEFINE_FUNC(c, "print", korb_print, 1);
}
...
/* iter 61: accessors used by the parse-end walker (parse.c) to
...
/* Set by parse.c after parsing PM_PROGRAM_NODE — the number of
...
create_context(int frames, int funcs)
{
    ...
    koruby_gc_sp_limit = c->env + (stack_bytes / sizeof(VALUE));
...
common_build_flags_and_link(void)
{
     ...
     * Absolute paths are baked at build time via -DKORUBY_PRECISE_DIR etc.
...
{
    koruby_update_sp_bodies_from_cc();

    if (ast) {
        astro_cs_compile(ast, koruby_current_source_file);
    }
    for (uint32_t i = 0; i < code_repo.size; i++) {
        if (code_repo.entries[i].skip_specialize) continue;
        NODE *body = code_repo.entries[i].body;
...
{
    // Use system() for simplicity — code_store/ is koruby-managed and
    // this only runs when the user explicitly asks via --ccs.
    int rc = system("rm -rf code_store");
    if (rc != 0) {
        fprintf(stderr, "koruby: --ccs: rm -rf code_store failed (rc=%d)\n", rc);
    }
}
...
main(int argc, char *argv[])
{
    ...
    // koruby_precise's own parser for sample-specific flags
    // (--ccs / -s / -b / -j).
    ...
    if (bcfg.version_requested) {
        printf("koruby_precise (ASTro %s)\n", ASTRO_VERSION);
        return 0;
    }

    // Translate framework flags into koruby's internal OPTION.
    // koruby pattern (docs/v2_spec.md §3.4): --aot-compile always implies a
    // run — bake from the post-parse code repo, then execute.  (The v1
    // mapping routed bare --aot-compile to a compile_only flag that no code
    // path consumed, so it baked nothing — bug fixed in v2.)
    //   --plain        → OPTION.plain (ignore code store)
    //   --aot-compile  → OPTION.compile_first (bake + run)
    //   --pg-compile   → OPTION.pg_at_exit (run + PGSD bake)
    if (bcfg.plain)        OPTION.plain         = true;
    if (bcfg.aot_compile)  OPTION.compile_first = true;
    ...
    //
    // koruby_precise: precise mark&sweep (gc.c), initialized in create_context.
    ...
    if (!OPTION.plain) {
        ...
        (void)astro_cs_load(ast, koruby_current_source_file);
    }
    ...
    if (!OPTION.compile_only) {
        ...
        if (UNLIKELY(r.state == RESULT_RAISE)) {
            /* CRuby-style uncaught-exception report: the raise value is a
             * pre-formatted "msg (Klass)" String (M0; real exception
             * objects come with rescue in M1). */
            fprintf(stderr, "%s: ", koruby_current_source_file ? koruby_current_source_file : "-e");
            koruby_print_value_tos(c, stderr, r.value);
            fputc('\n', stderr);
            aro_gc_fini(c);
            return 1;
        }
        if (getenv("KORUBY_BENCH_ELAPSED")) {
            fprintf(stderr, "__ELAPSED__ %.6f\n", elapsed);
        }
        (void)elapsed;
