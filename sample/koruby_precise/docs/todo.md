...
### VALUE_ARRAY の走査が容量ぶん = 実質 conservative
...
これは容量 (capa) であって、持ち主 (`KorbArray` / `KorbHash`) の `len` では
ない。items オブジェクト自身は論理長を知らないので、こう書くしかなかった、
という形。結果として **`len` を超えた予備スロットも GC のエッジとして走査
される**。

2026-09-07 に実測した例 (`p "x".ljust(4096).size` の prelude 中):

```
Array 0x7c7eb3802808 len=1 capa=4 items=0x7c7eb3802998 (4 要素ぶん走査)
   e0=0x7c7eb3801030 heap type=8 (Class)
   e1=0                nil
   e2=0x10000          ← ポインタでない生の値
   e3=0x7c7eb3802370 heap type=8 (Class)
```

`ARO_GC_VISIT_EDGE` のフィルタは「8 の倍数かつ非 0」だけなので、`0x10000` は
ポインタ候補として `forward_payload` に届く。**precise GC のつもりの箇所に
conservative な経路が混ざっている**、というのが実態。

いまは `forward_payload` が「アリーナ外はそのまま返す」ので実害は出ない
(この不変条件をいったん壊したのが 2026-09-07 に直した SEGV で、
[done.md](./done.md) 参照)。ただし残っている危険は 2 つ:

1. 予備スロットのゴミが偶然 from 空間のアドレスに見えると、GC がそれを
   オブジェクトとして forward しようとする。`gc_size` の健全性チェックに
   引っかかって "GC BUG: corrupt gc_size" で止まるか、最悪コピーが走る。
2. 死んだ参照を生かし続ける (`len` を超えたスロットに残った旧要素が回収
   されない)。

**候補となる直し方**:

- (a) 持ち主の側で `len` ぶんだけ回る。`KORB_OBJ_ARRAY` / `KORB_OBJ_HASH` の
  case が `items` の先頭 `len` (Hash は `len*2`) 要素を明示的に visit し、
  `KORB_OBJ_VALUE_ARRAY` 自体は no-scan にする。items の forward は
  `ARO_GC_VISIT_EDGE_PTR` で既にやっているので生存は保たれる。
  **要確認**: VALUE_ARRAY を持つのが Array / Hash だけか
  (`korb_runtime.c:3283` など他の確保箇所を洗う必要がある)。
- (b) `len` が減る経路 (pop / shift / delete / clear / replace / truncate) で
  予備スロットを 0 で埋める。確保時は `korb_alloc` が 0 埋めしているので、
  縮む側だけ塞げばよい。ただし漏れやすい。

まずは (a) の前提調査から。どちらにしても `0x10000` がどこで書かれたかは
未特定 (予備スロットに残ったものなのか、そこに直接書いた誰かがいるのか)。
