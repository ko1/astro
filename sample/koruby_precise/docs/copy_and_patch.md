# 即値埋め込み (パッチ束縛) の検討 — 2026-09-06

> **用語**: この文書が扱うのは **ロード時ノード束縛 (load-time node binding)** の
> うち、束縛先がコードである **パッチ束縛** である。もう一方の **間接束縛** (共有コード +
> ノードごとの表を `P[k]` で引く) とは上流のオペランド抽出を共有し、違うのは最終表現。
> 全体の設計と語彙は `docs/idea_code_store.md` §7.0 を参照。
>
> **文献との関係**: copy-and-patch (Xu & Kjolstad, **OOPSLA 2021**) から借りているのは
> **「穴を外部シンボルで表し、ELF 再配置レコードを頼りに実値でパッチする」技法だけ**で
> ある。論文の方式の他の要素は入っていない:
>
> - **stencil ライブラリが無い**。演算 × 型の組合せで小さなテンプレートを用意する仕掛けは
>   持たない。こちらが複製するのは形ごとに AOT 済みの SD 一本で、メソッド本体規模に
>   なりうる
> - **stencil の選択・連結が無い**。論文は実行時に AST を辿って stencil を選び tail-call で
>   数珠つなぎにするが、こちらは**構造ハッシュで選んでベイク時に C コンパイラに連結させて
>   いる** (SD の中で子は `static inline` な `SD_xxx()` として展開済み)。ローダは連結を
>   一切しない
>
> つまり「copy-and-patch の変種」ではなく「copy-and-patch のパッチ技法を使った」が正確。
> 選択と連結の時期という点ではむしろ対照的である。
