...
> うち、束縛先がコードである **パッチ束縛** (= copy-and-patch) である。もう一方の
> **間接束縛** (共有コード + ノードごとの表を `P[k]` で引く) と上流を共有し、分かれるのは束縛先だけ。
> 全体の設計と語彙は `docs/idea_code_store.md` §7.0 を参照。文献の copy-and-patch は
> Xu & Kjolstad, **OOPSLA 2021**。ASTro のパッチ束縛はその核心技法 (stencil を複製して
> 再配置で値を埋める) を使うが、論文の方式は AST ノード用 stencil の選択と連結まで
> 含むのに対し、こちらは形ごとに AOT 済みの大きな SD を複製する。「論文のアルゴリズム
> そのもの」ではなく一実装/変種と言うのが正確。
