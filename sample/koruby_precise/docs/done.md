...
### gc_copy: アリーナ外のポインタ候補をデリファレンスしない (SEGV 修正)

**症状**: ペイロードが 4096 バイト以上のオブジェクトを 1 個でも確保すると、
以後 `ASTRO_GC_STRESS=1` で SEGV する。文字列でも配列でも起き、境界は
`LARGE_THRESHOLD` (4096) ちょうど:
...
**原因**: エッジのフィルタ (`ARO_GC_VISIT_EDGE`) は「8 の倍数かつ非 0」なら
すべてポインタ候補として通す。だから走査対象スロットに入った素の整数も
`forward_payload` に届く。GC はそれを承知していて、from/to のどちらでもない
アドレスは「libc 由来の不死オブジェクト」としてそのまま返していた —
**触らずに**。

あとから足された「大サイズかどうか」の O(1) 判定
(`HDR_IS_LARGE((AroObjectHeader *)old_payload)`) が、この不変条件を破っていた。
`gc->large_head` が非 NULL になった瞬間から、アリーナ外の候補を全部
デリファレンスするようになる。実際に踏んだのは `0x10000` という値で、
これはポインタではなく、当然マップされていない。

大サイズが 1 個も無いうちは `gc->large_head == NULL` で分岐ごと飛ばされるため、
**最初の大サイズ確保まで潜伏する**。

**直し方**: 生きている大サイズのペイロードアドレスをソート済み配列
(`gc->large_addrs`) で持ち、二分探索で厳密に判定する。候補のメモリには
一切触らない。配列は確保時に挿入、掃除の頭で 1 パス詰め (HDR_MARKED を
リスト掃除が消す前に見る必要があるので順序が要る)、realloc で移動したら
張り替え、破棄で解放。

from/to の中にいる場合の分岐も同じ判定に揃えた。そちらは範囲判定だけで
大サイズと決めていたので、to 空間の普通のオブジェクトを大サイズと誤認し、
`payload - 8` に `next_gray` を書いて手前のオブジェクトを壊しうる形だった
(こちらは実際に踏んではいない)。安価な範囲判定を前に置いてあるので、
cheney のホット経路の命令数は変わらない。

**検証**: 再現ケース全滅 (`ljust(4096)` / `Array.new(511,0)` / `"x"*5000` /
`join` / `ljust(200000)`)。大サイズが GC を跨いで生き残り内容も壊れないこと、
`make test` 4901 PASS (FAIL 1 は既存の `complex_pow_coerce`)、
ascheme_precise 179/179、baruby_precise 8/8 (`T_gc_big` / `T_gc_stress` 込み)。
...
### KORB_NOT_REF — 静的な指摘を注釈で閉じる仕掛け

`value_read_after_gc.ql` の指摘が偽陽性になるのは「**ここには参照が来ない**」
ときだけである。ならそれを直接主張すればよい:

```c
v = KORB_NOT_REF(v);      /* may-GC 呼び出しの直前に置く */
... may-GC 呼び出し ...
use(v);
```

**(a) CodeQL がこれを見て指摘を消す。** クエリは `korb_not_ref` から来た定義を
「動かない」として扱うので、注釈した地点は報告から消える — ベースラインの数字と
して居座り続けない。
**(b) 主張が誤りなら実行時に分かる。** `-DKORB_STALE_CHECK` ビルドでは同じ呼び出しが
値を検査する。**GC は要らない** — heap 値が到達しただけで反証になる。

設計上の要点が 2 つある。どちらも実測で分かった:

- **文ではなく代入にする。** `KORB_ASSERT_NOT_REF(v);` のような文の形は release
  ビルドで `((void)0)` に消える。**CodeQL の DB は release ビルドから作られる**ので、
  注釈ごと消えて解析から見えない (実測: 66 → 66 で落ちなかった)。
- **`korb_not_ref` は全ビルドで実関数にする。** release では恒等関数 (最適化で
  消える) だが、呼び出しの形が DB に残る。マクロが `(v)` に潰れる形では、代入は
  残っても「参照でない」という情報が残らない (実測: 66 → 67 と逆に増えた。
  SSA の定義が 1 つ増えるだけで「定義 → GC → 使用」の形は残るため)。

C は同一スコープの再宣言ができないので、新しい名前ではなく**同じ変数への代入**に
してある。

実測: 19 箇所に置いて **66 → 42 行**。release の DB に `korb_not_ref` の呼び出しが
19 件見えていることも確認した。

**ただしこの 19 箇所は残していない。** 実アプリ (rbTris) で回したところ 7 箇所で
heap 参照が到達し、主張が誤りだと分かったため。仕掛けだけ残して注釈は外した。

### 実アプリで回して分かったこと / 分からなかったこと

**「heap 参照が来た」は「バグ」ではない。** その注釈が誤りというだけで、実際に
GC を跨ぐかは別問題。7 件のうち精査した 3 件は:

| 地点 | 判定 |
|---|---|
| `node_eval.c` `EVAL_node_singleton_def` の `obj` (rbTris で 251 回) | **偽陽性**。直後に `obj = VALUE_REF_GET(recv);` と読み直している |
| `string.c` `korb_m_str_start_with` の `pv` | **偽陽性**。`korb_coerce_to_str(c, slots, &pv)` がポインタ経由で更新する |
| `regexp.c` `korb_m_re_new` の `src` | **形は本物** (`korb_send_impl` の後に `korb_re_arg_type(src)`)。ただし実行時には確定できなかった |

`regexp.c` を確定できなかった経緯に、道具の限界が 2 つ出た:

1. **確保しないメソッドを呼んでも GC は起きない。** STRESS は「確保ごと」なので、
   `def to_str; 42; end` のような本体では移動しない。確保する本体に変えても 0 件だった
2. **accessor カバレッジ**。`KORB_STALE_CHECK` は当初 `VAL2STR` / `VAL2ARY` /
   `VAL2HASH` / `VAL2OBJ` / `VAL2CLASS` の 5 つしか包んでおらず、`KORB_OBJ_TYPE(v)` や
   `KORB_STR_ENC(v)` のようにヘッダを直接読む形が素通りしていた。全 22 個の `VAL2*` と
   これらを包んで deref 数は 63,698 → 240,223 (約 4 倍) に増えたが、それでも 0 件

**そして自己テストができなくなった。** korb_try_coerce の修正は「rhs を in/out に
する」署名変更だったので、書き戻しの 1 行を消しても元のバグ状態と等価にならない。
1,200 件を検出した実測は元のコードに対するもので有効だが、**いまこの検査器が本物を
鳴らせることを確かめる手段が無い**。したがって現時点の「0 件」を安全の根拠には
できない。回帰用に「確実に stale になる」小さな仕掛けを別途用意するのが先。
...
`"\0" * 848640` が 1 バイトの memcpy 848,640 回だった。元が 1 バイトなら
`memset`、そうでなければ倍々コピーにして呼び出し回数を `cnt` から
`log2(cnt)` に。**2.67 ms → 0.35 ms** (848,640 要素、ネイティブ)。

`Array#fill` は要素ごとに配列ヘッダを 2 回読み直し、伸長判定をしていた。
ブロック無しかつ範囲が既存の配列に収まるときは確保が起きず配列も動かないので、
読み直しと判定をループの外へ。**0.76 ms → 0.17 ms** (CRuby の 0.25 を逆転)。
