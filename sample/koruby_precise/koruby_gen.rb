...
class KorubyNodeDef < ASTroGen::NodeDef
  class Node < ASTroGen::NodeDef::Node
    ...
    def build_children_dispatch(kids)
      ...
      # (the callee frame's EP / link / identity at base[-2]/base[-3]/base[-4]).
      # The cursor advances by cnt+HDR but only cnt children are dispatched; the
      # identity is filled by korb_invoke, the EP stays the zero written here.
      ...
      # would be misread as a heap pointer.
    ...
    def build_children_specializer(kids)
      ...
      # AOT path leaves the EP/link/identity unreserved and corrupts the callee frame.
      adv = if @option.include?('@framehdr')
              ...
              '          fprintf(fp, "    slots[-(intptr_t)%u - 3] = 0;\\n", _cnt);' + "\n" +
              '          ' + link_emit
            else
              'fprintf(fp, "    slots += %u;\\n", _cnt);'
            end
        ...
        <<~C.chomp
                  if (#{f}[_i]->head.flags.no_inline) {
                      fprintf(fp, "    { NODE *const _cn = (NODE *)HOLE_PTR(%u); slots[%d] = UNWRAP((*_cn->head.dispatcher)(c, _cn, slots)); }\\n",
                              astro_hole_alloc(_hf, &_h, ASTRO_HOLE_EMIT_AT, "u.#{@name}.#{kids.name}", (int32_t)_i),
                              (int)_i - (int)_cnt);
                  } else {
                      const uint32_t _k = astro_hole_alloc(_hf, &_h, ASTRO_HOLE_EMIT_AT, "u.#{@name}.#{kids.name}", (int32_t)_i);
                      const uint32_t _o = astro_hole_sub(_hf, &_h, "u.#{@name}.#{kids.name}", (int32_t)_i, #{f}[_i]);
      ...
      def build_specializer(name)
        ...
        if !child? && @type == 'VALUE'
          sym_ref = if pool_mode?
            "        fprintf(fp, \"        (VALUE)HOLE_U64(%u)\", astro_hole_alloc(_hf, &_h, ASTRO_HOLE_EMIT, \"u.#{name}.#{self.name}\", -1));\n"
          else
