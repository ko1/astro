...
/* korb_float_new takes (c, sp, d); sync at alloc happens inside, so the
 * cfunc body just forwards its sp. */
#define MATH1(name, fn) \
    static RESULT math_##name(CTX *c, int argc, VALUE *sp) { \
        return RESULT_OK(korb_float_new(c, sp, fn(num_d(sp[-1])))); \
    }
#define MATH2(name, fn) \
    static RESULT math_##name(CTX *c, int argc, VALUE *sp) { \
        return RESULT_OK(korb_float_new(c, sp, fn(num_d(sp[-2]), num_d(sp[-1])))); \
    }
...
static RESULT math_frexp(CTX *c, int argc, VALUE *sp) {
    ...
    korb_ary_push(pair, korb_float_new(c, sp, sig));
