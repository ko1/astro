...
static RESULT korb_m_ary_fetch(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    ...
    if (UNLIKELY(!korb_to_index(slots[0], &i))) {         /* coerce a non-Integer index via #to_int */
        ...
        if (UNLIKELY(cr.state != KORB_NORMAL)) return cr;
        if (!korb_to_index(iv2, &i)) return korb_raise_no_int(c, slots, slots[0]);
    }
...
static RESULT korb_int_shift_arg(CTX *c, VALUE *slots, VALUE_REF self, VALUE o, int dir, korb_sword_t *sh_out, bool *extreme, VALUE *ext_val) {
    ...
    *extreme = false;
    for (;;) {
        ...
        if (!KORB_OBJECT_P(o) || !korb_responds_to(c, o, mid))   /* shift keeps the "of X into Integer" wording, nil included (CRuby) */
    ...
    for (uint32_t j = 0; j < k; j++) {
        ...
        if (!korb_to_index(iv, &i)) {                 /* coerce a non-Integer index via #to_int */
            ...
            if (!korb_to_index(iv, &i))
                return korb_raise_no_int(c, slots, VALUE_SLICE_GET(a, j));
        }
        const KorbArray *ary = VAL2ARY(VALUE_REF_GET(self));   /* re-read after possible dispatch */
        VALUE e = KORB_NIL;
        if (i < 0) i += ary->len;
    ...
    if (KORB_RANGE_P(pos0)) {
        ...
        if (pos0 != KORB_NIL) {
            if (UNLIKELY(!korb_to_index(pos0, &beg))) {   /* coerce start via #to_int (self is a VALUE_REF; n is a value) */
                ...
                if (UNLIKELY(cr.state != KORB_NORMAL)) return cr;
                if (!korb_to_index(pos0, &beg)) return korb_raise_no_int(c, slots, pos0);
                n = VAL2ARY(VALUE_REF_GET(self))->len;
            }
        }
        ...
        if (pos1 != KORB_NIL) {
            ...
            if (UNLIKELY(!korb_to_index(pos1, &len))) {
                ...
                if (UNLIKELY(cr.state != KORB_NORMAL)) return cr;
                if (!korb_to_index(pos1, &len)) return korb_raise_no_int(c, slots, pos1);
                n = VAL2ARY(VALUE_REF_GET(self))->len;
            }
            have_len = true;
        }
...
static RESULT korb_m_hash_dig(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a) {
    ...
    for (uint32_t k = 0; k < VALUE_SLICE_LEN(a); k++) {
        ...
        else if (KORB_ARRAY_P(cur)) { korb_sword_t i; if (UNLIKELY(!korb_to_index(key, &i))) return korb_raise_no_int(c, slots, key); KorbArray *ar = VAL2ARY(cur); if (i < 0) i += ar->len; cur = (i < 0 || (uint32_t)i >= ar->len) ? KORB_NIL : korb_items_data(ar->items)[i]; }
...
static RESULT korb_m_hash_take(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a) {
    korb_sword_t n;
    if (UNLIKELY(!korb_to_index(VALUE_SLICE_GET(a, 0), &n))) return korb_raise_no_int(c, slots, VALUE_SLICE_GET(a, 0));
...
static RESULT korb_hash_xform_keys(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    ...
    if (UNLIKELY(!has_arg && block == NULL))
        { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "transform_keys", 14)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
...
/* transform_keys! — CRuby renames pair by pair over a snapshot of the entries,
 * so a `break` out of the block leaves the already-renamed keys in place and the
 * rest of the hash untouched (rebuilding into a temp would lose both). */
...
static RESULT korb_m_hash_transform_keys_b(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    ...
    const bool has_arg = VALUE_SLICE_LEN(a) >= 1;
    if (has_arg && !KORB_HASH_P(VALUE_SLICE_GET(a, 0)))
        return korb_raise(c, slots, KORB_E_TYPE, 0, "no implicit conversion of %s into Hash", korb_coerce_name(c, VALUE_SLICE_GET(a, 0)));
    KORB_CHECK_FROZEN(c, slots, VALUE_REF_GET(self));
    slots[0] = has_arg ? VALUE_SLICE_GET(a, 0) : KORB_NIL;      /* rename map (rooted) */
    slots[1] = UNWRAP(korb_ary_new(c, slots + 1, VAL2HASH(VALUE_REF_GET(self))->len * 2));
    VALUE_REF pairs = VALUE_REF_AT(&slots[1]);                  /* flat [k, v, …] snapshot */
    for (uint32_t i = 0; ; i++) {
        const KorbHash *h = VAL2HASH(VALUE_REF_GET(self));
        if (i >= h->len) break;
        slots[2] = korb_items_data(h->items)[2 * i];
        slots[3] = korb_items_data(h->items)[2 * i + 1];
        CHECK(korb_ary_push_val(c, slots + 4, pairs, slots[2]));
        CHECK(korb_ary_push_val(c, slots + 4, pairs, slots[3]));
    }
    slots[2] = UNWRAP(korb_hash_new(c, slots + 2, 8));          /* the keys produced so far */
    VALUE_REF seen = VALUE_REF_AT(&slots[2]);
    const uint32_t np = VAL2ARY(VALUE_REF_GET(pairs))->len;
    for (uint32_t i = 0; i + 1 < np; i += 2) {
        slots[3] = korb_items_data(VAL2ARY(VALUE_REF_GET(pairs))->items)[i];       /* old key */
        slots[4] = korb_items_data(VAL2ARY(VALUE_REF_GET(pairs))->items)[i + 1];   /* value */
        slots[5] = slots[3];                                                       /* new key (default = old) */
        bool mapped = false;
        if (slots[0] != KORB_NIL) {
            const int32_t j = korb_hash_find(VAL2HASH(slots[0]), slots[3]);
            if (j >= 0) { slots[5] = korb_items_data(VAL2HASH(slots[0])->items)[2 * j + 1]; mapped = true; }
        }
        if (!mapped && block != NULL) {
            RESULT r = korb_block_yield(c, slots + 6, block, def_env, &slots[3], 1, cself);
            if (UNLIKELY(r.state != KORB_NORMAL)) return r;     /* break/raise: keep the partial rename */
            slots[5] = r.value;
        }
        if (korb_hash_find(VAL2HASH(VALUE_REF_GET(seen)), slots[3]) < 0) {   /* the old key was not re-created → drop it */
            slots[6] = slots[3];
            CHECK(korb_m_hash_delete(c, slots + 7, self, VALUE_SLICE_MAKE(&slots[6], 1), NULL, NULL, NULL));
        }
        CHECK(korb_hash_set(c, slots + 6, self, VALUE_REF_AT(&slots[5]), slots[4]));
        slots[6] = slots[5];
        CHECK(korb_hash_set(c, slots + 7, seen, VALUE_REF_AT(&slots[6]), KORB_NIL));
    }
    return RESULT_OK(VALUE_REF_GET(self));
}
...
static RESULT korb_m_hash_flatten(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a) {
    ...
    if (VALUE_SLICE_LEN(a) >= 1 && VALUE_SLICE_GET(a, 0) != KORB_NIL) {
        ...
        if (UNLIKELY(!korb_to_index(dv, &depth))) {       /* coerce depth via #to_int, else TypeError */
            ...
            if (UNLIKELY(cr.state != KORB_NORMAL)) return cr;
            if (!korb_to_index(dv, &depth)) return korb_raise_no_int(c, slots, VALUE_SLICE_GET(a, 0));
        }
    }
    uint32_t n = VAL2HASH(VALUE_REF_GET(self))->len;
...
static RESULT korb_m_hash_sort_by(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "sort_by", 7)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
...
static RESULT korb_m_hash_filter_map(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "filter_map", 10)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
    VALUE_REF dst = SLOTS_PUSH(slots, UNWRAP(korb_ary_new(c, slots, 4)));
    for (uint32_t i = 0; ; i++) {
        const KorbHash *h = VAL2HASH(VALUE_REF_GET(self));
...
static RESULT korb_m_hash_partition(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "partition", 9)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
...
static RESULT korb_m_hash_find(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "find", 4)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
    for (uint32_t i = 0; ; i++) {
        const KorbHash *h = VAL2HASH(VALUE_REF_GET(self));
        if (i >= h->len) break;
...
static RESULT korb_m_hash_find_all(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "find_all", 8)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
    VALUE_REF dst = SLOTS_PUSH(slots, UNWRAP(korb_ary_new(c, slots, 4)));
    for (uint32_t i = 0; ; i++) {
        const KorbHash *h = VAL2HASH(VALUE_REF_GET(self));
...
static RESULT korb_m_hash_group_by(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    (void)a; if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "group_by", 8)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
...
static RESULT korb_m_hash_find_index(CTX *c, VALUE *slots, VALUE_REF self, VALUE_SLICE a, NODE *block, VALUE *def_env, VALUE *cself) {
    ...
    if (UNLIKELY(block == NULL)) { slots[0] = VALUE_REF_GET(self); slots[1] = ID2SYM(korb_intern(c->vm, "find_index", 10)); return korb_send(c, slots + 2, korb_intern(c->vm, "__to_enum_sized", 15), 0, 1); }
    uint32_t np = korb_entry_params_cnt(block);
    for (uint32_t i = 0; ; i++) {
        const KorbHash *h = VAL2HASH(VALUE_REF_GET(self));
        if (i >= h->len) break;
