...
static RESULT ary_aset(CTX *c, int argc, VALUE *sp) {
    ...
    if (argc == 2 && FIXNUM_P(argv[0])) {
        ...
        korb_ary_aset(c, sp, self, i, argv[1]);
    ...
    if (argc == 3 && FIXNUM_P(argv[0]) && FIXNUM_P(argv[1])) {
        ...
        if (!SPECIAL_CONST_P(val) && BUILTIN_TYPE(val) != T_ARRAY) {
            ...
            val = argv[2];
            if (RTEST(rt)) {
                VALUE coerced = UNWRAP(korb_funcall(c, sp, val, korb_intern("to_ary"), 0, NULL));
...
static RESULT ary_join(CTX *c, int argc, VALUE *sp) {
    ...
    if (korb_ary_len(self) == 0) return RESULT_OK(korb_str_new(c, sp, "", 0));
    ...
    if (argc < 1 || NIL_P(argv[0])) {
        ...
        sep = UNWRAP(korb_funcall(c, sp, argv[0], korb_intern("to_str"), 0, NULL));
...
static RESULT ary_zip(CTX *c, int argc, VALUE *sp) {
    ...
    bool has_block = korb_block_given(c);
    KORB_ARY_YIELD_FRAME(c, fc, korb_ary_new_capa(c, sp, argc));   /* cooked[] */
    fc.last_match = fc.last_line;             /* cooked-sources array */
    fc.last_line  = sp[-argc - 1];            /* source receiver */
    KORB_ARY_YIELD_FRAME(c, fr,
        has_block ? Qnil : korb_ary_new_capa(c, sp, korb_ary_len(sp[-argc - 1])));  /* result */
    for (int j = 0; j < argc; j++) {
        ...
        if (!SPECIAL_CONST_P(v) && BUILTIN_TYPE(v) == T_ARRAY) {
            ...
            VALUE rt_ary = UNWRAP(korb_funcall(c, sp, v, korb_intern("respond_to?"), 1,
                                        (VALUE[]){ korb_id2sym(korb_intern("to_ary")) }));
            if (RTEST(rt_ary)) {
                VALUE r2 = UNWRAP(korb_funcall(c, sp, v, korb_intern("to_ary"), 0, NULL));
                if (!SPECIAL_CONST_P(r2) && BUILTIN_TYPE(r2) == T_ARRAY) cooked = r2;
            }
            if (NIL_P(cooked)) {
                VALUE rt_a = UNWRAP(korb_funcall(c, sp, v, korb_intern("respond_to?"), 1,
                                          (VALUE[]){ korb_id2sym(korb_intern("to_a")) }));
                if (RTEST(rt_a)) {
                    VALUE r2 = UNWRAP(korb_funcall(c, sp, v, korb_intern("to_a"), 0, NULL));
                    if (!SPECIAL_CONST_P(r2) && BUILTIN_TYPE(r2) == T_ARRAY) cooked = r2;
                }
            }
            if (NIL_P(cooked)) {
                ...
                VALUE rt_each = UNWRAP(korb_funcall(c, sp, v, korb_intern("respond_to?"), 1,
                                             (VALUE[]){ korb_id2sym(korb_intern("each")) }));
                if (RTEST(rt_each)) {
                    VALUE args[2] = { v, INT2FIX(korb_ary_len(fc.last_line)) };
                    VALUE arr = UNWRAP(korb_funcall_r(c, sp,
        ...
        korb_ary_push(c, sp, fc.last_match, cooked);   /* nil tuple element if unset */
    }
    ...
    for (long i = 0; i < n; i++) {
        ...
        if (has_block) {
            RESULT _y = korb_yield(c, sp + 1, 1, &sp[0]);
            if (_y.state != KORB_NORMAL) { c->current_frame = fc.prev; return _y; }
        } else {
...
static RESULT ary_each_slice(CTX *c, int argc, VALUE *sp) {
    ...
    KORB_ARY_YIELD_FRAME(c, fr, has_block ? Qnil : korb_ary_new(c, sp));
    fr.last_match = sp[-argc - 1];
...
static RESULT ary_concat(CTX *c, int argc, VALUE *sp) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (SPECIAL_CONST_P(arg) || BUILTIN_TYPE(arg) != T_ARRAY) {
            RESULT tr = korb_funcall_r(c, sp + argc, arg, korb_intern("to_ary"), 0, NULL);
            if (tr.state != KORB_NORMAL) { return tr; }
            arg = tr.value;
    ...
    for (int i = 0; i < argc; i++) {
        for (long j = 0; j < lens[i]; j++) {
            ...
            korb_ary_push(c, sp + argc, sp[-argc - 1], korb_ary_items(o)[j]);
        }
    }
    return RESULT_OK(sp[-argc - 1]);
}
...
static RESULT ary_transpose(CTX *c, int argc, VALUE *sp) {
    ...
    if (korb_ary_len(self) == 0) return RESULT_OK(korb_ary_new(c, sp));
    ...
    for (long j = 0; j < n_outer; j++) {
        ...
        if (!SPECIAL_CONST_P(inner)) {
            VALUE rt = UNWRAP(korb_funcall(c, sp + 3, inner, korb_intern("respond_to?"), 1,
                                    (VALUE[]){ korb_id2sym(korb_intern("to_ary")) }));
            if (RTEST(rt)) {
                VALUE r = UNWRAP(korb_funcall(c, sp + 3, inner, korb_intern("to_ary"), 0, NULL));
...
static RESULT ary_mul(CTX *c, int argc, VALUE *sp) {
    ...
    if (!FIXNUM_P(arg) && BUILTIN_TYPE(arg) != T_STRING) {
        VALUE rt_str = UNWRAP(korb_funcall(c, sp, argv[0], korb_intern("respond_to?"), 1,
                                    (VALUE[]){ korb_id2sym(korb_intern("to_str")) }));
        if (RTEST(rt_str)) {
            VALUE s = UNWRAP(korb_funcall(c, sp, argv[0], korb_intern("to_str"), 0, NULL));
            if (!SPECIAL_CONST_P(s) && BUILTIN_TYPE(s) == T_STRING) {
                arg = s;
            }
        } else {
            VALUE rt_int = UNWRAP(korb_funcall(c, sp, argv[0], korb_intern("respond_to?"), 1,
                                        (VALUE[]){ korb_id2sym(korb_intern("to_int")) }));
            if (RTEST(rt_int)) {
                VALUE iv = UNWRAP(korb_funcall(c, sp, argv[0], korb_intern("to_int"), 0, NULL));
...
static RESULT ary_assoc(CTX *c, int argc, VALUE *sp) {
    ...
    for (long i = 0; i < alen; i++) {
        ...
        if (!SPECIAL_CONST_P(e) && BUILTIN_TYPE(e) == T_ARRAY) {
            ...
            RESULT _ta = korb_funcall(c, sp + 1, e, korb_intern("to_ary"), 0, NULL);
        ...
        VALUE r = UNWRAP(korb_funcall(c, sp + 1, korb_ary_items(ea)[0], korb_intern("=="), 1, eq_args));
...
static RESULT ary_rassoc(CTX *c, int argc, VALUE *sp) {
    ...
    for (long i = 0; i < alen; i++) {
        ...
        VALUE eq_args[1] = { argv[0] };
        VALUE r = UNWRAP(korb_funcall(c, sp + 1, korb_ary_items(ea)[1], korb_intern("=="), 1, eq_args));
        if (RTEST(r)) return RESULT_OK(sp[0]);
    }
    return RESULT_OK(Qnil);
}
...
static RESULT ary_product(CTX *c, int argc, VALUE *sp) {
    ...
    KORB_ARY_YIELD_FRAME(c, fr,
        c->current_block ? Qnil : korb_ary_new(c, sp));   /* result */
    fr.last_match = korb_ary_new_capa(c, sp, n);          /* sources */
    korb_ary_push(c, sp, fr.last_match, sp[-argc - 1]);
    for (int i = 0; i < argc; i++) korb_ary_push(c, sp, fr.last_match, (sp - argc)[i]);
...
static RESULT ary_first_n(CTX *c, int argc, VALUE *sp) {
    ...
    if (!FIXNUM_P(arg) && (SPECIAL_CONST_P(arg) || BUILTIN_TYPE(arg) != T_BIGNUM)) {
        if (!SPECIAL_CONST_P(arg)) {
            RESULT _tr = korb_funcall(c, sp, arg, korb_intern("to_int"), 0, NULL);
            if (_tr.state == KORB_RAISE) {
