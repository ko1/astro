...
static RESULT nil_to_a(CTX *c, int argc, VALUE *sp)  { c->sp = sp; return RESULT_OK(korb_ary_new(c, c->sp)); }
static RESULT nil_to_h(CTX *c, int argc, VALUE *sp)  { c->sp = sp; return RESULT_OK(korb_hash_new(c, c->sp)); }
