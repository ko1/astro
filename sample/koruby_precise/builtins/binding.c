...
static struct korb_binding *binding_alloc_from(CTX *c, VALUE *sp, VALUE recv) {
    ...
    b->outer_names_cnt = 0;
        if (!NIL_P(src->extra_vars) && BUILTIN_TYPE(src->extra_vars) == T_HASH) {
            b->extra_vars = korb_hash_new(c, c->sp_top);
    ...
    if (c->current_eval_program_body) {
        ...
        if (eval_names) {
            if (NIL_P(b->extra_vars)) b->extra_vars = korb_hash_new(c, sp);
    ...
    if (c->running_block) {
        if (NIL_P(b->outer_vars)) b->outer_vars = korb_hash_new(c, sp);
...
static RESULT kernel_binding_cfunc(CTX *c, int argc, VALUE *sp) {
    ...
    return RESULT_OK((VALUE)binding_alloc_from(c, sp, self));
}
...
static RESULT binding_receiver(CTX *c, int argc, VALUE *sp) {
    ...
    return RESULT_OK(((struct korb_binding *)self)->self);
}
...
static RESULT binding_local_variable_defined_p(CTX *c, int argc, VALUE *sp) {
    ...
    if (SPECIAL_CONST_P(self) || BUILTIN_TYPE(self) != T_DATA) return RESULT_OK(Qfalse);
    struct korb_binding *b = (struct korb_binding *)self;
    bool ok = false;
    ID name_id = binding_arg_to_id(c, argv[0], &ok);
    if (!ok) return RESULT_OK(Qfalse);
    return RESULT_OK(KORB_BOOL(binding_find_slot(b, name_id) >= 0));
}
...
RESULT binding_eval_via(CTX *c, struct korb_binding *b, VALUE *argv, int argc) {
    ...
    if (b->fp) {
        if (NIL_P(b->extra_vars)) b->extra_vars = korb_hash_new(c, c->sp_top);
...
static RESULT binding_source_location(CTX *c, int argc, VALUE *sp) {
    ...
    korb_ary_push(arr, korb_str_new_cstr(c, sp, file));
...
static RESULT binding_eval_cfunc(CTX *c, int argc, VALUE *sp) {
    VALUE self = sp[-argc - 1];
    VALUE *argv = sp - argc;
...
static VALUE binding_dup_clone_impl(CTX *c, VALUE self, bool preserve_frozen, int argc, VALUE *argv) {
    ...
    if (!NIL_P(src->extra_vars) && BUILTIN_TYPE(src->extra_vars) == T_HASH) {
        dst->extra_vars = korb_hash_new(c, c->sp_top);
        struct korb_hash *sh = (struct korb_hash *)src->extra_vars;
        for (struct korb_hash_entry *e = sh->first; e; e = e->next) {
...
static RESULT binding_dup_cfunc(CTX *c, int argc, VALUE *sp) {
    ...
    return RESULT_OK(binding_dup_clone_impl(c, self, false, argc, argv));
}
