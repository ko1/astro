...
RESULT str_class_new(CTX *c, int argc, VALUE *sp) {
    ...
    /* See ary_class_new comment: stage [r, argv...] on sp, bump c->sp_top
...
static RESULT str_concat_one(CTX *c, VALUE *sp, VALUE self, VALUE arg) {
    ...
    /* Park self / arg / tmp in sp[0..2] across korb_str_new + korb_str_concat. */
    sp[0] = self;
    sp[1] = arg;
    sp[2] = Qnil;
    if (FIXNUM_P(sp[1])) {
        ...
        if (cp < 0) {
            ...
            return korb_raise(c, (struct korb_class *)eR, "%ld out of char range", cp);
        }
        if (cp <= 0x7f) {
            char ch = (char)cp;
            sp[2] = korb_str_new(c, sp + 3, &ch, 1);
        ...
        sp[2] = korb_str_new(c, sp + 3, buf, len);
        korb_str_concat(c, sp + 3, sp[0], sp[2]);
        return RESULT_OK(sp[0]);
    }
    if (SPECIAL_CONST_P(sp[1]) || BUILTIN_TYPE(sp[1]) != T_STRING) {
        ...
        VALUE eT = korb_const_get(KORB_VM(c)->object_class, korb_intern("TypeError"));
        return korb_raise(c, (struct korb_class *)eT, "no implicit conversion to String");
    }
    if (sp[1] == sp[0]) {
        struct korb_string *src = (struct korb_string *)sp[1];
        sp[2] = korb_str_new(c, sp + 3, src->ptr, src->len);
        korb_str_concat(c, sp + 3, sp[0], sp[2]);
        return RESULT_OK(sp[0]);
    }
...
static RESULT str_chomp_compute(CTX *c, VALUE *sp, VALUE self, int argc, VALUE *argv, long *out_n) {
    ...
    *out_n = n;
    ...
    if (argc < 1) {
        ...
        if (NIL_P(rs) || SPECIAL_CONST_P(rs) || BUILTIN_TYPE(rs) != T_STRING) {
            if (n >= 2 && s->ptr[n-2] == '\r' && s->ptr[n-1] == '\n') { *out_n = n - 2; return RESULT_OK(Qnil); }
            if (n >= 1 && (s->ptr[n-1] == '\n' || s->ptr[n-1] == '\r')) { *out_n = n - 1; return RESULT_OK(Qnil); }
            return RESULT_OK(Qnil);
        }
        arg = rs;
        goto have_str;
    ...
    if (SPECIAL_CONST_P(arg) || BUILTIN_TYPE(arg) != T_STRING) {
        ...
        if (RTEST(rt)) {
            VALUE r = UNWRAP(korb_funcall(c, sp, arg, korb_intern("to_str"), 0, NULL));
            if (!SPECIAL_CONST_P(r) && BUILTIN_TYPE(r) == T_STRING) {
                arg = r;
                goto have_str;
            }
...
static RESULT str_chomp(CTX *c, int argc, VALUE *sp) {
    ...
    long n = 0;
    CHECK(str_chomp_compute(c, sp, self, argc, argv, &n));
    /* str_chomp_compute may coerce its arg (GC) — re-read self. */
    return RESULT_OK(korb_str_new(c, sp, ((struct korb_string *)sp[-argc - 1])->ptr, n));
}
...
static RESULT str_rindex(CTX *c, int argc, VALUE *sp) {
    ...
    VALUE arg = argv[0];
    if (SPECIAL_CONST_P(arg) || BUILTIN_TYPE(arg) != T_STRING) {
        arg = UNWRAP(str_coerce_arg(c, sp, arg));
    }
...
static RESULT str_gsub(CTX *c, int argc, VALUE *sp) {
    ...
    sp[0] = self;
    sp[1] = argv[0];
    sp[2] = (argc >= 2) ? argv[1] : Qnil;
    sp[3] = 0;
    sp[4] = Qnil;
    sp[3] = korb_str_new(c, sp + 5, "", 0);
    struct korb_string *s = (struct korb_string *)sp[0];
    long start = 0, i = 0;
    long ms, ml;
    while (str_find_pat(sp[1], s, i, &ms, &ml)) {
        korb_str_concat(c, sp + 5, sp[3], korb_str_new(c, sp + 5, s->ptr + start, ms - start));
        s = (struct korb_string *)sp[0];
        if (argc >= 2 && BUILTIN_TYPE(sp[2]) == T_STRING) {
            struct korb_string *r = (struct korb_string *)sp[2];
            korb_str_concat(c, sp + 5, sp[3], korb_str_new(c, sp + 5, r->ptr, r->len));
            s = (struct korb_string *)sp[0];
            ...
            else korb_str_concat(c, sp + 5, sp[3], korb_to_s(c, sp + 5, sp[4]));
            s = (struct korb_string *)sp[0];
        }
        i = ms + (ml > 0 ? ml : 1);
        start = i;
    }
    korb_str_concat(c, sp + 5, sp[3], korb_str_new(c, sp + 5, s->ptr + start, s->len - start));
    return RESULT_OK(sp[3]);
}
...
static RESULT str_sub(CTX *c, int argc, VALUE *sp) {
    ...
    struct korb_string *s = (struct korb_string *)sp[0];
    long ms, ml;
    if (!str_find_pat(sp[1], s, 0, &ms, &ml)) return RESULT_OK(korb_str_dup(c, sp + 5, sp[0]));
    sp[3] = korb_str_new(c, sp + 5, s->ptr, ms);
    s = (struct korb_string *)sp[0];
    if (argc >= 2 && BUILTIN_TYPE(sp[2]) == T_STRING) {
        struct korb_string *r = (struct korb_string *)sp[2];
        korb_str_concat(c, sp + 5, sp[3], korb_str_new(c, sp + 5, r->ptr, r->len));
    ...
    korb_str_concat(c, sp + 5, sp[3], korb_str_new(c, sp + 5, s->ptr + ms + ml, s->len - ms - ml));
    return RESULT_OK(sp[3]);
}
...
static VALUE str_tr_impl(CTX *c, VALUE self, int argc, VALUE *argv, bool squeeze) {
    if (argc < 2 || BUILTIN_TYPE(argv[0]) != T_STRING || BUILTIN_TYPE(argv[1]) != T_STRING)
        return korb_str_dup(c, c->sp_top, self);
    struct korb_string *s = (struct korb_string *)self;
    ...
    out[w] = 0;
    return korb_str_new(c, c->sp_top, out, w);
}
...
static RESULT str_insert(CTX *c, int argc, VALUE *sp) {
    ...
    VALUE other = argv[1];
    if (SPECIAL_CONST_P(other) || BUILTIN_TYPE(other) != T_STRING) {
        other = UNWRAP(str_coerce_arg(c, sp, other));
    }
...
static RESULT str_coerce_arg(CTX *c, VALUE *sp, VALUE arg) {
    ...
    if (!SPECIAL_CONST_P(arg)) {
         ...
         * re-read.  Park it at sp[0] so visit_roots forwards it; the
         * sp+1-framed funcalls publish that depth so the park stays scanned. */
        VALUE *const aroot = sp;
        aroot[0] = arg;
        RESULT _rt = korb_funcall(c, sp + 1, aroot[0], korb_intern("respond_to?"), 1,
                                (VALUE[]){ korb_id2sym(korb_intern("to_str")) });
        if (_rt.state != KORB_NORMAL) { return _rt; }
        if (RTEST(_rt.value)) {
            RESULT _r = korb_funcall(c, sp + 1, aroot[0], korb_intern("to_str"), 0, NULL);
            if (_r.state != KORB_NORMAL) { return _r; }
            VALUE r = _r.value;
            if (!SPECIAL_CONST_P(r) && BUILTIN_TYPE(r) == T_STRING) {
                return RESULT_OK(r);
...
static RESULT str_delete_prefix(CTX *c, int argc, VALUE *sp) {
    ...
    if (argc < 1) return RESULT_OK(korb_str_new(c, sp, ((struct korb_string *)self)->ptr,
                                       ((struct korb_string *)self)->len));
    VALUE arg = UNWRAP(str_coerce_arg(c, sp, argv[0]));
    ...
    if (p->len <= s->len && memcmp(s->ptr, p->ptr, p->len) == 0 &&
        ...
        return RESULT_OK(korb_str_new(c, sp, s->ptr + p->len, s->len - p->len));
    return RESULT_OK(korb_str_new(c, sp, s->ptr, s->len));
}
...
static RESULT str_delete_suffix(CTX *c, int argc, VALUE *sp) {
    ...
    if (p->len <= s->len && memcmp(s->ptr + cut, p->ptr, p->len) == 0 &&
        ...
        return RESULT_OK(korb_str_new(c, sp, s->ptr, cut));
    return RESULT_OK(korb_str_new(c, sp, s->ptr, s->len));
}
...
static RESULT str_each_line(CTX *c, int argc, VALUE *sp) {
    ...
    fr.last_line = has_block ? Qnil : korb_ary_new(c, sp);
    ...
    for (long i = 0; i < len; i++) {
        if (ptr[i] == '\n') {
            VALUE line = korb_str_new(c, sp, ptr + start, i - start + 1);
            if (has_block) {
                RESULT _y = korb_yield(c, sp, 1, &line);
                if (_y.state != KORB_NORMAL) { c->current_frame = fr.prev; return _y; }
            } else {
                korb_ary_push(c, sp, fr.last_line, line);
            }
            start = i + 1;
        }
    }
    if (start < len) {
        VALUE line = korb_str_new(c, sp, ptr + start, len - start);
        if (has_block) {
            RESULT _y = korb_yield(c, sp, 1, &line);
            if (_y.state != KORB_NORMAL) { c->current_frame = fr.prev; return _y; }
        } else {
...
RESULT _enc_name(CTX *c, int argc, VALUE *sp) {
    ...
    if (UNDEF_P(name) || NIL_P(name)) return RESULT_OK(korb_str_new_cstr(c, sp, "UTF-8"));
