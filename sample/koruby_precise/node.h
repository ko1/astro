...
// koruby_ary_push fast-path inline (iter 73): split out the slow
// realloc path into koruby_ary_push_grow in node.c, keep the
...
static inline void
koruby_ary_push(CTX *c, VALUE_REF av_ref, VALUE_REF x_ref)
{
    ...
    koruby_ary_push_grow(c, av_ref, x_ref);
}
...
// Enable optional NodeHead fields used by koruby
...
// the framework); koruby keeps sp_body excluded from HORG so the call
...
//   sp[0] = UNWRAP(KORUBY_EVAL_ARG(c, lv, sp + 1));
// iter 61 dropped fp from the dispatcher signature — only sp remains.
#define KORUBY_EVAL_ARG(c, n_node, new_sp) ((*n_node##_dispatcher)(c, n_node, new_sp))
