...
void
koruby_update_sp_bodies_from_cc(void)
{
    for (struct pg_call_link *l = all_pg_call_nodes_head; l; l = l->next) {
        if (l->cc->serial == 0 || l->cc->body == NULL) continue;
        // sp_body is excluded from HORG (koruby_gen.rb hash_call kind=horg)
...
callsite_resolve(void)
{
    ...
    while (cs) {
        ...
        if (body == NULL) {
            fprintf(stderr, "undefined method '%s' for main (NoMethodError)\n", cs->name);
            exit(1);
        }
        if (cs->target_field) {
        ...
        // sp_body is excluded from HORG (see koruby_gen.rb), so the
...
alloc_binop(struct transduce_context *tc, pm_constant_id_t name, NODE *lhs, NODE *rhs)
{
    if (0) {}
        ...
        // Each eval still allocates a fresh KoString (str_lit semantics),
    ...
    switch (node->type) {
      ...
      case PM_CALL_NODE: {
          ...
          // syntax Ruby-ish while avoiding any class / dispatch table.
              ...
              // since koruby has no methods on receivers other than the
      ...
      case PM_RETURN_NODE: {
          ...
          if (n->arguments) {
              ...
              // koruby's int64-only VALUE, so we just take the first.
...
show_help(void)
{
    fprintf(stderr, "koruby_precise [options] [script] [argv...]\n\n");
    fprintf(stderr, "koruby_precise-specific options:\n");
    ...
    for (int i = 1; i<argc; i++) {
        ...
        switch (arg[1]) {
          ...
          case 'j':
            ...
            fprintf(stderr,
                    "koruby_precise: JIT (-j) is unwired post-fork; "
...
const char *koruby_current_source_file = NULL;

NODE *
PARSE(int argc, char *argv[])
{
    const char *fname = parse_option(argc, argv);
    koruby_current_source_file = fname;
