...
module Enumerable
  ...
  def each_slice(n); n = n.to_int unless n.is_a?(Integer); raise ArgumentError, "invalid slice size" unless n > 0; r = []; s = []; each { |x| s << x; if s.size == n; r << s; s = []; end }; r << s unless s.empty?; if block_given?; r.each { |sl| yield sl }; nil; else; r.each; end; end
  def each_cons(n); n = n.to_int unless n.is_a?(Integer); raise ArgumentError, "invalid size" unless n > 0; r = []; buf = []; each { |x| buf << x; if buf.size == n; r << buf.dup; buf.shift; end }; if block_given?; r.each { |cc| yield cc }; nil; else; r.each; end; end
  def zip(*others); os = others.map { |o| o.to_a }; r = []; i = 0; each { |x| row = [x]; os.each { |o| row << o[i] }; r << row; i += 1 }; if block_given?; r.each { |row| yield row }; nil; else; r.each; end; end
