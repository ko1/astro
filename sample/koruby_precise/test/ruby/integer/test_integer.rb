...
# Tests for Integer#gcd, #lcm, #digits.


def test_gcd
  assert_equal 6, 12.gcd(18)
  assert_equal 1, 7.gcd(13)
...
def test_gcdlcm
  assert_equal [6, 36], 12.gcdlcm(18)
end

def test_digits
  assert_equal [3, 2, 1], 123.digits
  assert_equal [0], 0.digits
  assert_equal [3, 2, 1, 0, 0, 1], 100123.digits
end

def test_digits_base
  assert_equal [0, 1, 1], 6.digits(2)  # 0b110 → [0, 1, 1] (LSB first)
  assert_equal [3, 2, 1], 0x123.digits(16)
end
...
TESTS = %i[
  test_basic test_compare test_bitops
  test_bit_access test_bignum test_divmod test_times
  test_step test_upto_downto test_float_coerce test_chr
  test_to_s_base test_div_mod_edges test_bitops_negative test_fixnum_boundary
  test_compare_int_float test_int_methods test_even_odd test_pow
  test_to_s_corner test_step_with_block_break test_spaceship_returns_nil test_gcd
  test_lcm test_gcdlcm test_digits test_digits_base
]
