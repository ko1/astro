# pathname.rb — vendored from CRuby's pathname_builtin.rb (the pure-Ruby core
# that replaced ext/pathname's C implementation) plus the find / FileUtils /
# tmpdir extensions from CRuby's lib/pathname.rb.  Kept verbatim so the
# semantics match; koruby-specific edits are marked with "koruby:".
...
class Pathname
  ...
  # koruby: the methods below live in CRuby's pathname.c.  These are the POSIX
  # ('/' only, no drive letters, case-sensitive) Ruby equivalents — the same
  # shapes the original pure-Ruby pathname.rb used before the C rewrite.
  SEPARATOR_LIST = "/"
  SEPARATOR_PAT = /\//
  ...
  def sub(pattern, *rest, &block)
    path = block ? @path.sub(pattern, *rest) { |*a| block.call(*a) } : @path.sub(pattern, *rest)
    self.class.new(path)
  end

  # 't.tmp'.sub_ext('.txt') -> 't.txt';  'temp'.sub_ext('.txt') -> 'temp.txt'
  def sub_ext(repl)
  ...
  # chop_basename(path) -> [pre-basename, basename], or nil at a root/empty path
  ...
  def has_separator?(path)
    SEPARATOR_PAT.match?(path)
  end
  private :has_separator?
  ...
  def del_trailing_separator(path)
    if (r = chop_basename(path))
      pre, basename = r
      pre + basename
    elsif (m = /#{SEPARATOR_PAT}+\z/o.match(path))
      m.pre_match + File.dirname(path)[-1, 1]
    else
      path
    end
  end
