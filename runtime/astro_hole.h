...
// architecture backend asks for, so each hole survives as a relocation the
...
#define HOLE_U32(k) ((uint32_t)ASTRO_ARCH_HOLE_IMM(k))
#define HOLE_I32(k) ((int32_t)(uint32_t)ASTRO_ARCH_HOLE_IMM(k))
