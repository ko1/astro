...
#ifdef ASTRO_NODEHEAD_POOL
    // Everything up to the descriptors is the SD text.  A loader-only store
    // compiles this file with -DASTRO_SD_DESC_ONLY and keeps only the data:
    // the code it needs lives in op/<SD>.o, woven per node.
...
astro_cs_build_target(const char *extra_cflags, const char *target)
{
    ...
    fprintf(fp, "CFLAGS_DESC ?= -O0 -DASTRO_SD_DESC_ONLY\n");
