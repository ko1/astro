...
{
    return astro_cs_loader_only != 0;
}
#else
bool astro_cs_is_loader_only(void) { return false; }
#endif
...
astro_cs_set_preload(const char *path)
{
    ...
    // A loader-only preload store settles it for the process: its SDs have no
    // dispatchers either, so they must be woven the same way.
    if (astro_ld_preload_dir[0]) {
        char marker[ASTRO_CS_PATH_MAX];
        astro_cs_path(marker, sizeof(marker), astro_ld_preload_dir, "loader_only");
...
#if ASTRO_LOADER_SUPPORTED
        // The store says whether it was baked loader-only; the env var is how
        // a bake turns that on in the first place.
        char marker[ASTRO_CS_PATH_MAX];
...
#if ASTRO_LOADER_SUPPORTED
    // Loader-only store: there is no dispatcher to bind — the shape is baked
    // iff its descriptor is there.  Keep the interpreter dispatcher; the
    // embedder weaves the body (astro_cs_instantiate) when it is first
    // reached.  Binding the name and the pool here is what makes that possible.
    if (astro_cs_loader_only && astro_cs_dlsym_desc(sym_name)) {
        n->head.dispatcher_name = alloc_dispatcher_name(n);
        astro_cs_bind_pool(n, sym_name);
...
#endif
    // node.h comes first either way: the descriptors are offsetof expressions,
    // so even a descriptor-only build needs the node layout.
    fprintf(fp, "#include \"%s/node.h\"\n", astro_cs.src_dir);
    fprintf(fp, "\n");
#ifdef ASTRO_NODEHEAD_POOL
    // Everything from here to the descriptors is the SD text.  A loader-only
    // store compiles this file with -DASTRO_SD_DESC_ONLY and keeps only the
    // data: the code it needs lives in op/<SD>.o, woven per node.
    fprintf(fp, "#ifndef ASTRO_SD_DESC_ONLY\n");
#endif
...
astro_cs_build_target(const char *extra_cflags, const char *target)
{
    ...
    fprintf(fp, "\n");
    // Descriptor-only objects: the same sources with the SD text preprocessed
    // away, so all.so carries the shape descriptors and no code at all.
    fprintf(fp, "CFLAGS_DESC ?= $(CFLAGS) -DASTRO_SD_DESC_ONLY\n");
    fprintf(fp, "DOBJS = $(patsubst c/%%.c,od/%%.o,$(SRCS))\n");
    fprintf(fp, "od/%%.o: c/%%.c | od\n");
    fprintf(fp, "\t$(CC) $(CFLAGS_DESC) -c $< -o $@\n");
    fprintf(fp, "od:\n");
    fprintf(fp, "\tmkdir -p od\n");
    fprintf(fp, "\n");
#endif
#if ASTRO_LOADER_SUPPORTED
...
{
#if ASTRO_LOADER_SUPPORTED
    // Record the store's kind before the make: a consumer must not read a
...
#if ASTRO_LOADER_SUPPORTED
    // ASTRO_CS_PATCH=1: also build the loader-path objects (op/).  A
    // loader-only store has no other code, so they are not optional there.
    const char *patch = getenv("ASTRO_CS_PATCH");
    if (astro_cs_loader_only || (patch && patch[0] && strcmp(patch, "0") != 0))
