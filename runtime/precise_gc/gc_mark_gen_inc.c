...
typedef struct ASTroGC {
    ...
    LargeObj *large_head;
    struct AroObjectHeader **young_objs;
...
static void gray_push(ASTroGC *gc, AroObjectHeader *h);
...
new_page(ASTroGC *gc, int class_idx)
{
    ...
    for (size_t i = 0; i < n_slots; i++) {
        ...
        /* Freelist holds SLOT POINTERS; link is overlaid at h + sizeof(header). */
...
static AroObjectHeader *slab_alloc(ASTroGC *gc, size_t payload_size, int class_idx)
{
...
static AroObjectHeader *large_alloc(ASTroGC *gc, size_t payload_size)
{
...
aro_gc_wb(CTX *c, void *holder, VALUE *slot, VALUE v)
{
    ...
    if (inc_marking) {
        VALUE old = *slot;
        if (AROH_IS_GC_OBJECT(old)) mark_value_satb(gc, old);
    }
    *slot = v;
