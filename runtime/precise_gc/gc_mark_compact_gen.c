...
aro_gc_init(CTX *c)
{
    ...
    if (getenv("BARUBY_GC_STRESS")) {
        ...
        fprintf(stderr, "[aro_gc=mark_compact_gen] STRESS mode: collect on every alloc\n");
    }
...
pretenure_alloc(CTX *c, size_t payload_size, size_t total)
{
    ...
    if (tenured_top + total > tenured_end) {
        major_gc(c);
        if (tenured_top + total > tenured_end) {
            fprintf(stderr, "aro_gc=mark_compact_gen: OOM tenured (need %zu)\n", total);
            abort();
        }
    }
...
nursery_collect_cold(CTX *c, size_t total)
{
    ...
    if (young_top + total > young_active_base + YOUNG_BYTES) {
        major_gc(c);
        if (young_top + total > young_active_base + YOUNG_BYTES) {
            fprintf(stderr, "aro_gc=mark_compact_gen: OOM young (need %zu)\n", total);
            abort();
        }
    }
}
