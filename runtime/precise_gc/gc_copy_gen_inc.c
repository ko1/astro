// gc_copy_gen_inc.c — backend #7: PLACEHOLDER (= identical to copy_gen).
//
// ⚠ iter 35 honesty fix: this backend was originally a clone of
// copy_gen with no actual incremental logic.  After the N-survive rewrite
// (= 4-面 + age) it remains a clone of the new copy_gen.  Future work
// would add inc_marking + SATB + incremental Cheney; until then bench
// harness treats copy_gen and copy_gen_inc as the same data point.
//
// Below: identical N-survive logic to gc_copy_gen.c — see that file for
...
nursery_collect_cold(CTX *c, size_t total)
{
    ...
    if (young_top + total > young_active_base + YOUNG_BYTES) {
        major_gc(c);
        if (young_top + total > young_active_base + YOUNG_BYTES) {
            fprintf(stderr, "baruby_gc=copy_gen_inc: OOM young (need %zu)\n", total);
            abort();
        }
    }
}
