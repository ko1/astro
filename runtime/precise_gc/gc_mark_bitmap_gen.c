...
#define HDR_AGE_SHIFT    1
#define HDR_AGE_MASK     ((uint16_t)0x0006u)
...
aro_gc_init(CTX *c)
{
    ...
    if (getenv("BARUBY_GC_STRESS")) {
        ...
        fprintf(stderr, "[aro_gc=mark_bitmap_gen] STRESS mode: collect on every alloc\n");
    }
...
new_page(ASTroGC *gc, int class_idx)
{
     ...
     * pointers (= AroObjectHeader *).  FreeSlot.next link is stored at
     * `slot + sizeof(AroObjectHeader)` (= overlapping the first sample
...
check_edge_for_young(void *ctx, void **slot)
{
    ...
    if (AROH_IS_GC_OBJECT(v)) {
        ...
        if (HDR_IS_FREE(h)) return;
        if (!get_old(gc, h)) gc->scan_saw_young = true;
    }
}
...
gc_collect_minor(CTX *c)
{
    ...
    /* N-survive remset compaction: scan each entry + keep if any young child
     * remains.  Drop (= clear dirty) otherwise.  See gc_mark_gen.c for
     * design notes. */
    if (remset_overflow) {
        remset_cnt = 0;
        for (int sc = 0; sc < NUM_SIZE_CLASSES; sc++) {
            ...
            for (Page *pg = page_head[sc]; pg; pg = pg->next) {
                ...
                char *slot = (char *)pg + SLOTS_REGION_OFFSET;
                for (size_t i = 0; i < n; i++, slot += sb) {
                    if (!bm_get(pg->old_bm, i)) continue;
                    if (!bm_get(pg->dirty_bm, i)) continue;
                    AroObjectHeader *h = (AroObjectHeader *)slot;
    ...
    /* Promote-time WB: scan newly-promoted obj's edges, push to remset if young child. */
    for (size_t i = 0; i < promoted_cnt; i++) {
        ...
        if (gc->scan_saw_young) {
            Page *pg; size_t idx;
            if (locate(h, &pg, &idx)) bm_set(pg->dirty_bm, idx);
            else { LargeObj *lo = find_large(gc, h); if (lo) lo->dirty = true; }
            remset_push(gc, h);
        }
    }
    promoted_cnt = 0;
...
gc_collect_major(CTX *c)
{
    ...
    in_minor = false;
    remset_cnt = 0;
    /* N-survive WB invariant: dirty bit set ⇒ obj is in remset_buf.
     * After remset_cnt = 0, all dirty bits MUST be cleared so WB
     * re-fires for tenured→young writes that happen post-major.
     * (Original mark_bitmap_gen avoided this by clearing dirty for every
     * remset entry during minor compaction; N-survive keeps some entries
     * dirty across minors, so we must reset bits here.) */
    for (int sc = 0; sc < NUM_SIZE_CLASSES; sc++) {
        for (Page *pg = page_head[sc]; pg; pg = pg->next) {
            memset(pg->dirty_bm, 0, BITMAP_BYTES);
        }
    }
    for (LargeObj *lo = large_head; lo; lo = lo->next) lo->dirty = false;
    remset_overflow = false;

    AROH_VISIT_ROOTS(c, gc, mark_edge);
    process_gray(gc);

    /* Finalize pass: live = marked, dead = !marked. */
    aro_gc_finalize_walk(c);
...
/* Liveness via per-page bitmap (or LargeObj flags for large objs):
 *   minor: live = old || marked, dead = neither.
 *   major: live = marked, dead = !marked. */
void *
