...
- `sample/` — 各種言語実装サンプル
  ...
  - メインストリーム: `naruby` (Ruby サブセット, JIT 対応), `abruby` (Ruby サブセット, CRuby C 拡張), `koruby` (Ruby+, optcarrot 動かすことを目標), `luastro` (Lua サブセット), `pystro` (Python 3 サブセット, GMP bignum + class + try/except), `astr` (R サブセット, libgc + tagged VALUE + numeric/string/list), `ascheme` (Scheme), `wastro` (Wasm), `jstro` (JavaScript), `astocaml` (OCaml サブセット), `castro` (C サブセット), `asom` (SOM), `astrogre` (Ruby/Onigmo 互換 regex エンジン + `are` という grep CLI), `nuq` (jq クローン: JSON フィルタ言語 + 70+ builtin)
- `docs/`
