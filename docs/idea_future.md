...
- **最高性能はフル意味論側にある** — gcc が見えるのは 1 SD に展開された C の範囲だけで、
  言語レベルの不変条件 (tag/boxing、escape、deopt 可能点) は見えない。unboxing・
  ノード横断 guard 除去・CPS・lvar の C 変数化は「body の意味論を持つ側」にしかできない。
  上限は「現状 < アノテーション < 限定解析 < フル意味論」で単調に上がり、追い詰めた先は
  Truffle / RPython の領分 (フレームワークが実装言語の意味論を所有する) に収束する
- ただし **content-addressed code store と「C ソースを IR にして backend を外注」は
  意味論の所有と直交** — フル意味論に踏み込んでも、変換後の C を吐いて Horg/Hopt で
  共有する ASTro の骨格は保てる。backend まで自前の Truffle/RPython に対する差別化は
  踏み込んだ後も残る。論点は性能-コスト曲線の knee の位置で、「意味論なし + gcc で
  どこまで詰められるか」の実証が現サンプル群
