...
| **load/store の typed-pointer 化で TBAA 解放** (`(VALUE *)pv->i = vv` ではなく `int64_t *ip = (int64_t *)pv; *ip = vv;`) | castro `perf.md §12` | castro: quicksort -27%。VALUE union 経由だと gcc が「他の VALUE * (= fp[]) と alias 可能性あり」と保守判定してしまうのを回避 | union slot 表現の言語全般（castro / ascheme / luastro 等）。グローバル配列への store の後に frame slot を re-load しなくて済むケースが増える |
...
（`local_get → box_get` 等）でハッシュ整合が崩れるのが原因だった。

#### 4.5.1 parser 後処理 kind 書き換えと OPTIMIZE キャッシュの整合（fixed）

**症状（luastro deltablue / richards / json で顕在化）**:
`all.so` がロード済みの 2 回目以降の起動で、captured slot を持つ
inner function を呼ぶと "attempt to index a unknown value" などで
落ちる。1 回目の AOT bake (`-c`) では発生しない。

**原因**: 各 `ALLOC_*` が `OPTIMIZE` を呼ぶ → `astro_cs_load` が
hash を計算してキャッシュ + dispatcher を `SD_<hash>` にパッチ。
これは parse 中に行われるため、capture rewrite
(`pf_finalize_local_refs` で `node_local_get → node_box_get`) が
走る前にハッシュが確定し、`SD_<HASH_local_get>` に dispatcher が
張り付く。書き換え後は kind は `box_get` だが dispatcher は
`local_get` 用 SD のまま → captured slot の読み出しで生 `LuaBox*`
が値として返り、後続の `box[key]` で indexing 失敗。
1 回目（fresh）は `all.so` がまだ無いので `astro_cs_load` が
no-op、ハッシュは bake 時に初めて確定するためズレが起きない。

**解決（言語非依存パターン）**: parser がノードの kind を後から
書き換える可能性がある言語では、

1. **全 NODE 追跡**: `node_allocate` で割り付けた NODE を
   グローバル配列に積む。
2. **parse 終了時に invalidate + 再 OPTIMIZE**: kind 書き換え
   パスが終わった後で、追跡済みの全 NODE について
   `head.flags.has_hash_value = false` / `head.flags.is_specialized
   = false` をリセットし、`dispatcher` を kind の
   `default_dispatcher` に戻し、もう一度 `OPTIMIZE(n)` を呼ぶ。

luastro `node.c::luastro_reoptimize_all`（`PARSE_lua` 末尾で 1 回
だけ呼ぶ）。AOT-cached 起動時に追加コストはかかるが parse 中の
mutation を持つ言語では他に手がない。

**教訓**: `OPTIMIZE` の hash キャッシュは「ノードが **構造的に
最終形** になっていること」を前提にしている。parse 中に kind や
子ポインタを書き換えるパスがあるなら、

- (a) **OPTIMIZE を遅延**して全 mutation 後に呼ぶ、または
- (b) **mutation 後に invalidate + 再 OPTIMIZE**、

のいずれかを必ず入れる。子ポインタの swap も同罪
（fold / 定数畳み込み等）— 親のキャッシュが stale 化する。
...
| Tail-return 畳み込み（return ノードを seq/if に折り畳む → setjmp 不要）| castro `perf.md §1` | castro: fib_big 1.8×、ack/loop_sum/tak は setjmp ゼロ。**castro 自身は §10 RESULT 移行で全廃**したが、setjmp/longjmp を持つ言語実装で同種の最適化対象 |
| **`node_call_static`: NODE 子オペランドを介した leaf-helper インライン化** + Tarjan SCC で recursive callee は除外 | castro `perf.md §11` | 非自己再帰 callee の SD chain が caller TU に static inline 展開、gcc -O3 が透視。recursive な callee を walk すると body 内の自己 `:call` が emit する `extern SD_<self>` 宣言と framework が emit する static inline def が衝突するので SCC で除外 | 関数定義が `def f { ... }` 形で書ける全言語、leaf helper が hot path にいる場合 |
| **caller-allocated stack VLA frame for SROA**（call 時に `VALUE F[local_cnt]` を caller stack に確保し callee に渡す。SD chain がフル inline されると `&F` は escape せず、gcc SROA が F[i] を register に昇格）| wastro `node.def:1672` 由来、castro `perf.md §13` で採用 | castro: sieve の inner mark loop が **5 命令 → 3 命令** (i, n がレジスタ化、メモリストア消失)、quicksort -25%、nqueens -29%。`SPECIALIZE` override で `local_cnt` を **literal baked** にして `VALUE F[6]` の固定サイズ配列形にするのが SROA 起動の鍵。`VALUE F[local_cnt]` の VLA syntax だと gcc SROA が部分的にしか走らない | スロット型 frame を持つ全言語、特に AOT 化された静的 SD chain |
...
2. **再帰インライン**（gcc が踏める展開を ASTro でも踏めるように）
3. **独自ローダー**（`code_store_quirks.md §自前ローダーで解消する`）

§4.5.1（parser 後処理 mutation と OPTIMIZE キャッシュの整合）は
luastro `luastro_reoptimize_all` で解決済み。
...
**系として: hot path に数命令足す変更の cycles / fps を、別ビルド同士で比べない。**
配置の差が変更の効果より大きい。正しい対照は「**同一バイナリの中に実行時スイッチを入れ、
同じ code_store で ON/OFF を測る**」。同日の koruby の例: C メソッドの窓をフレームにする変更は
別ビルド比較で optcarrot AOT cycles +6.18% / fps −2.5% と出たが、同一バイナリ ON/OFF では
**命令 +0.12% / fps +0.81% (劣化なし)** で、差は全部配置だった。
スイッチは「新規に足した分だけ」を切ること: 最初に既存の機構まで止めてしまい、OFF 側が
別のコールドパスに落ちて +188M 命令になり、**ON の方が速いという逆の結論**が出かけた。
