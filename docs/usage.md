...
- `sample/anml/` — [MinCaml](https://esumii.github.io/min-caml/) (a monomorphic ML subset).  Demonstrates a from-scratch **Hindley–Milner type inferer** (destructive unification, `type.c`), **de Bruijn frame** variable resolution (`node_lref(depth, idx)` resolved at parse time), boxed-float/closure/tuple/array values, and a differential harness vs the system `ocaml` (a two-line prelude reconciles the only surface differences).  Ships a self-contained reimplementation reference (`docs/mincaml_impl_spec.md`).
