...
| `astro_build_extract_flags(&argc, argv, &cfg)` | `astro_build.h` | Pre-scan argv (order-free) for framework flags, removing them in place; also loads `ASTRO_BUILD_OPTS` env.  Stops at the first non-flag positional. |
| `astro_build_load_env_opts(&cfg)`          | "                    | Parse `ASTRO_BUILD_OPTS` standalone (called by extract_flags). |
| `astro_build_config_dispose(&cfg)`         | "                    | Free heap arrays in cfg (extra_cflags / extra_ldflags from env). |
| `astro_print_build_help(fp)`               | "                    | Emit the framework's flag-list section.  Sample's usage() should call this. |
| `ASTRO_VERSION` (macro)                    | "                    | Version string embedded in every sample; used by `--version` handlers. |
...
- `sample/ancaml/` — [MinCaml](https://esumii.github.io/min-caml/) (a monomorphic ML subset).  Demonstrates a from-scratch **Hindley–Milner type inferer** (destructive unification, `type.c`), **de Bruijn frame** variable resolution (`node_lref(depth, idx)` resolved at parse time), boxed-float/closure/tuple/array values, tail-call elimination (trampoline) + leaf-frame `alloca`, and a differential harness vs the system `ocaml` (a two-line prelude reconciles the only surface differences).  Ships a self-contained reimplementation reference (`docs/mincaml_impl_spec.md`).  Part of the **An\*** series ("ASTro Nutshell" — bite-sized pedagogical languages; cf. `anpy`).
- `sample/anlox/` — [Lox](https://craftinginterpreters.com/) (the *Crafting Interpreters* teaching language: dynamic typing, closures, single-inheritance classes).  Demonstrates a parse-time **resolver** assigning `(depth, slot)` to locals while globals stay late-bound by name, bound methods via a `this`-frame, and `super` dispatch.  Tested with the book's canonical **`// expect:` annotated format** (self-contained — no system Lox exists), optionally diffed against a reference via `ANLOX_REF`.  Variable-arity children use index-addressed side-tables, so `--build` (AST embedding) is unsupported; interpreter + `--aot-compile` work.  Part of the **An\*** series.
