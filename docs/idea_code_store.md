...
**代償**: 縮退先が pool ではなくインタプリタになる。`op/` を全部消すと出力は正しいまま
インタプリタで走る (0/1482 織り込み・193 failed)。`--compiled-only` の保証を保つため、
織り込みに失敗した body に poison dispatcher を据える処理を足した (`@noinline` body は従来どおり除外)。
wasm / `--build` / x86-64 以外は L が無いので、そこは P のまま。
