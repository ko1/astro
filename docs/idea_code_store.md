...
## 7. ロード時ノード束縛 (load-time node binding) — パッチ束縛と間接束縛 (設計、2026-09-06)

### 7.0 用語

SD は木の**形**に対しては特殊化されているが、ノードごとの**値**に対しては特殊化されて
いない。その値をロードの時点でノードから取り出して結びつけるのが、この節の技法である。
呼び名を **ロード時ノード束縛 (load-time node binding)** とする。曖昧さを潰したいときは
「ノード値のロード時束縛」と書く。

| 語 | 意味 |
|---|---|
| **ロード時ノード束縛** (load-time node binding) | 技法全体。ノード由来の値をロード時に結びつける |
| **束縛オペランド** (bound operand) | 結びつける値。形の特殊化を生き残ったオペランド |
| **束縛記述子** (binding descriptor) | `SD_<h>_desc`。ノードからその値をどう作るかのレシピ |
| **パッチ束縛** (patch binding) | 束縛先がコード。ノードごとのコードに即値で焼く (copy-and-patch) |
| **間接束縛** (indirect binding) | 束縛先がデータ。共有コード + ノードごとの表を `P[k]` で引く (辞書渡し) |

**上流のオペランド抽出は完全に共通で、違うのは最終表現だけ**である。`node.def`、
生成 `.c`、hash、束縛記述子、`astro_hole_walk` はすべて同一で、分かれるのは
`runtime/astro_hole.h` の `#if` 一枚:

```c
#if defined(ASTRO_SD_PATCH)   #define HOLE_U64(k) ASTRO_ARCH_HOLE_IMM(k)   /* 即値 */
#elif defined(ASTRO_SD_POOL)  #define HOLE_U64(k) P[k]                     /* 表引き */
```

(「違いは束縛先だけ」と書くと言いすぎになる。パッチ束縛にはコード複製・ELF 再配置・
W^X・ISA バックエンド・コード同一性の分裂が付いてくる。同じなのは上流であって
実装の重さではない。名前も片方が手段・片方が結果で少しずれるが、それぞれの側で
一番はっきりした語を採った。)

型クラスの実装で言えば、間接束縛が辞書渡し、パッチ束縛が単相化にあたる。
間接束縛があることで、ISA バックエンドを書けない環境 (wasm、W^X) でも同じ生成物と
同じ記述子がそのまま使える。

パッチ束縛は copy-and-patch (Xu & Kjolstad, OOPSLA 2021) から**「穴を外部シンボルで表し
ELF 再配置レコードを頼りに実値でパッチする」技法だけ**を借りている。stencil ライブラリも
stencil の選択・連結も入っていない — 選択は構造ハッシュが、連結はベイク時の C コンパイラ
(`static inline` な子 SD) が済ませている。よって「copy-and-patch の変種」ではなく
「その技法を使った」と書く。詳細は `sample/koruby_precise/docs/copy_and_patch.md`。

コード内部の識別子は歴史的に穴 (`astro_hole_*`, `HOLE_U64`) を使っている。抽象として
なら穴は間接束縛にも成立する — 「テンプレートの未束縛パラメータ」と読めば `P[k]` は
「穴 k を表のスロットで実現したもの」である。使わない理由は偽だからではなく、
**copy-and-patch のコードに開いた空白を強く連想させ、パッチ束縛に偏った名前になる**
から。よって**内部実装の語としてはそのまま残し、説明はロード時ノード束縛で統一する**。

### 7.0.1 束縛の時期はポリシーである

「ロード時」と呼ぶが、束縛を**いつ**行うかは選べる (`KORUBY_INSTANTIATE`、`main.c` の
`korb_inst_mode`):

- `all` — ロード時に全 body を束縛する。走らない body のぶんも払う (上限の測定用)
- `first` / `second` / `hot` — トランポリン経由で遅延し、**実際に到達した body だけ**
  束縛する。`second` は「走る body の 46% は 1 回しか走らない」ぶんを丸ごと省く (§7.7)

遅延できることは欠点ではなく利点で、必要なものだけ束縛できる。極端には、呼ばれた
瞬間にロードごと行ってもよい。動的リンカの lazy PLT binding が同じ形である —
`ld.so` は初回呼び出しでシンボルを解決するが、それを理由に load-time binding という
語が偽になるとは言わない。時期はスケジューリングの選択であって、機構の性質ではない。

### 7.0.2 なぜノードを供給源にするのか

束縛記述子は**ノードを読むプログラム**であって、そのノードが**いつできたか**を問わない。
実行中に `def` / `eval` / `Class.new` / `define_method` が作ったノードでも、形が既知なら
その場で束縛オペランドの供給源になる。別途メタデータを持つ必要も、シリアライズも、
コンパイル時の表も要らない。

これは通常の AOT が持てない性質である。**AOT は本来「ビルド時に存在しなかったコード」を
助けられない**が、この構造なら形さえ見たことがあれば助けられる。

散らばって見える以下の性質は、すべてこの 1 つの性質の裏返しである:

- **ストアが intern 順に依存しない** (`@sym`、1b327c48)。シンボルリテラルの VALUE は
  プロセスごとに違うので焼けない。逆に「値はノードから取る」を守る限り順序非依存になる
- **プログラム A で焼いたストアがプログラム B を助ける**。ASTro の R (Reusable) が
  成立する理由そのもの
- **`--build` の exe はパースせずに AST を組み直す**のに、同じ記述子がそのまま効く

間接束縛ではこれが文字通りの形で出る。`n->head.pool` はノードに付いた環境なので、
`(dispatcher, pool)` はノードに対するクロージャである。部分評価の言葉で言えば
**AST ノードが残余環境そのもの**で、ベイク時の部分評価器が形を消費したあと、残った値は
元からあった場所 (ノード) に置いたままでよい。「値をどこかに移す」工程が存在しない。

構造の縁が出るのは 1 点だけ。実行中に生まれ続けるノードに対し、パッチ束縛の
インスタンスは immortal なので (§7 の `astro_cs_instantiate`、活性化が中で走りうる)
無限には作れない。間接束縛はノードごとに配列を足すだけなので、この点では素直に伸びる。
...
**しかし `all` は実行されない body まで織り込んでいた**。optcarrot 180f の 60M ディスパッチ全体で
1 回でも呼ばれる body は 2027 中 **485** で、`all` の 1983 のうち約 1500 は使われないコードの
コピーだった。そこで **`first` (初回ディスパッチで織り込む)** を足した (`KORUBY_INSTANTIATE=first`)。
全 body にワンショットのトランポリンを置き、最初に到達したときだけ `astro_cs_instantiate` して
トランポリンを外す。sp4 5 ラウンド交互 (`logs/desc/first-vs-all-sp4.txt`):

| | fps max / median | 織り込み | 起動 (`-e 'p 1'`) |
|---|---|---|---|
| pool のみ | 208.3 / 208.1 | — | 21.9 ms |
| `all` | 232.0 / 231.2 | 1983 body / 14.7 MB | 50.9 ms |
| **`first`** | **234.0 / 233.5** | 492 body / 5.4 MB | 27.0 ms |
| `hot` 0.005 | 223.1 / 222.8 | 29 body / 215 KB | 21.9 ms |

`first` が最速で、常駐は `all` の 1/3、起動は pool +5 ms。到達した body だけを織り込めば十分。

**閾値を 1 より大きくすると OSR が要る (2026-09-07)**。`second` (2 回目のディスパッチで織り込む。
1 ビットを `head.hash_opt` に置くだけ) も入れて測った。optcarrot 180f で 1 回でも呼ばれる body は
2027 中 485、うち**ちょうど 1 回が 224 (46.2%)** なので、`second` の織り込みは 492 → 261 body、
常駐 4,968 → 1,798 KB、起動 25.7 → 21.0 ms まで落ちる。それでも既定にはしない:

- ディスパッチ回数は実行時間の代理指標として壊れている。中でループを回す body は 1 回しか
  呼ばれなくても実行時間を独占する。1 回呼ばれて 2000 万回ループする例で `first` 0.05 s に対し
  `second` 0.31 s (**6 倍**)。optcarrot の 2% 差 (230.1 → 225.5 fps) も同じ理由。
- **回収する手段が無い**。バックエッジを数えて熱いと分かっても、実行中のアクティベーションは
  差し替えられない (制御が entry に戻らない)。OSR の領域で、body が AST 部分木を評価する 1 個の
  C 関数である以上、対応づけられる PC が無い。ループの内側は SD にインライン展開されていて
  別単位で差し替えることもできない。
- `first` が正しいのは、**判断する時点 (ディスパッチ直前) と安全に差し替えられる唯一の時点が
  一致している**から。閾値を 1 より大きくした瞬間にその一致が壊れる。
- `second` は 2 回目に到達しなかった body をインタプリタに残すので、`--compiled-only` の保証も
  静かに破れる (poison は織り込み失敗時にしか発火しない)。

`hot` (窓の終わりに割合で選ぶ) は同じ理由に加えて、**L のみのストアでは成立しない**。選ばれ
なかった body の逃げ場が pool ではなくインタプリタになるため、optcarrot で 172.5 fps
(織り込み無し = 69.8、`first` = 230.1)。P 構成でのみ意味を持つ。

### 7.7.4 L のある環境で P を作らない (`ASTRO_CS_LOADER_ONLY`, 2026-09-07)

`first` まで来ると pool 版 SD は「初回ディスパッチまでの姿」でしかない。L が使える環境では
それも要らないので、**pool 版をそもそもビルドしない**構成を用意した。

- 生成: 記述子をファイル末尾にまとめ (`astro_hole_flush_descs`)、SD 本体を
  `#ifndef ASTRO_SD_DESC_ONLY` で囲む。同じ `.c` を `-DASTRO_SD_DESC_ONLY` でコンパイルすると
  記述子だけの小さな `.o` になる (`od/`)。
- ベイク: **ローダのある backend では既定でこれ**。`all.so` を `od/` からリンクし、`o/` は作らず、
  `op/` は必須。`ASTRO_CS_LOADER_ONLY=0` で従来の pool ビルドに戻る。ストアに `loader_only`
  marker を置くので、消費側は環境変数なしで同じ判断ができる (焼く意図と、手元のストアが何で
  あるかは別問題なので、コード上も別の値にしてある)。
- ロード: `SD_<h>` が無く `SD_<h>_desc` がある形は「焼いてあるが未ロード」。名前と pool だけ結び、
  dispatcher はインタプリタのまま返す。埋め込み側が初回ディスパッチで織り込む。

| | P あり | P 無し |
|---|---|---|
| program `all.so` (502 SD) | 3.66 MB | **280 KB** |
| preload `all.so` (1120 SD) | 7.28 MB | **588 KB** |
| `o/` オブジェクト | 502 | **0** |
| fps (sp4, max/median) | 230.4 / 229.8 (`first`) | 230.3 / 228.4 |
| 起動 (`-e 'p 1'`) | 20.2 ms | 25.4 ms |

速度は `P + first` と同じ (織り込まれたコードが同一なので当然)。差はサイズとビルド量である。

この構成では P は「無い」のが正常であって、縮退先ではない。`--compiled-only` は
「全部コンパイル済みで走った」の意味を保つ必要があるので、織り込みに失敗した body には
poison dispatcher を据える (`@noinline` body は従来どおり除外)。これは実行時コストゼロの
ゲートであって、pool の代わりの受け皿ではない。

L が無い環境 (wasm / `--build` / x86-64 以外) は従来どおり P で焼く。
...
### 7.7.5 織り込み失敗時にトランポリンが残るバグ (2026-09-08、修正済み)

`first` / `second` のトランポリンは「一度きり」の前提で、`n->head.dispatcher` を
`orig` に戻してから `astro_cs_instantiate` を呼ぶ。ところが**呼び出しキャッシュ**
(fat inline cache / call cache) は `body->head.dispatcher` を**複製して持つ**ので、
ノードの dispatcher を書き換えただけでは既存のキャッシュはトランポリンを指したまま
になる。これを無効化するのが `korb_dispatchers_swapped()` だが、これを
**織り込み成功時にしか呼んでいなかった**。

結果、織り込みに失敗する body は毎回トランポリンに再入し、その度に
`astro_cs_instantiate` → `astro_ld_obj_open` (`op/<SD>.o` の open 2 回) を実行する。
`class` を定義する小さなプログラム (32,000 回ループ) で:

| | 実行時間 | `first: … failed` |
|---|---|---|
| L (ローダのみストア) | 0.04 s | 0 |
| P (pool ストア) 修正前 | **0.20 s** | **32,003** ← ループ回数と一致 |
| P (pool ストア) 修正後 | 0.04 s | 4 |
| インタプリタ | 0.03 s | — |

L では織り込みが必ず成功するので 0 件、つまり**この不具合は P 構成でしか出ない**。
「AOT にすると遅くなる」形で出るので性能バグとして質が悪い。

修正は 2 点 (`sample/koruby_precise/main.c`):

- `korb_dispatchers_swapped()` を**無条件**に呼ぶ。トランポリンは成功・失敗に
  関わらず外れているので、キャッシュはどちらでも stale である。失敗した body の
  分だけ bump が増えるが、それは壊れていた経路の数と同じで無視できる。
- poison の条件から `astro_cs_is_loader_only()` を外し、**戻した dispatcher が
  インタプリタ既定かどうか**で判断する。ローダのみ判定はプロセス単位 (preload
  ストアの marker が決める) なので、pool な `code_store` + ローダのみな
  `preload_store` という混在構成で、pool SD を持つ body まで poison していた
  (`--compiled-only` が誤って abort する)。

**教訓**: dispatcher を複製して持つキャッシュがある以上、「ノードの dispatcher を
書き換える」コードは成功パスだけでなく**全ての経路で**キャッシュを無効化しないと
いけない。ワンショットのトランポリンは、外し損ねるとワンショットでなくなる。
