...
## 7. ロード時ノード束縛 (load-time node binding) — pool 束縛とパッチ束縛 (設計、2026-09-06)
...
| **プール束縛** (pool binding) | 束縛先がデータ。共有コード + ノードごとの表 (辞書渡し) |
| **パッチ束縛** (patch binding) | 束縛先がコード。ノードごとのコードに即値で焼く (copy-and-patch) |

**2 実装の違いは束縛先だけ**である。上流 — `node.def`、生成 `.c`、hash、束縛記述子、
