...
`first` が最速で、常駐は `all` の 1/3、起動は pool +5 ms。`hot` は起動を pool と同じに保てるが
定常性能で `first` に 11 fps 負ける。到達した body だけを織り込めば十分で、呼び出し回数で
さらに絞る意味は (この題材では) 無い。
...
- 生成: 記述子をファイル末尾にまとめ (`astro_hole_flush_descs`)、SD 本体を
  ...
  記述子だけの小さな `.o` になる (`od/`)。
- ベイク: `ASTRO_CS_LOADER_ONLY=1` で `all.so` を `od/` からリンクし、`o/` は作らず、`op/` は必須。
  ストアに `loader_only` marker を置くので、消費側は環境変数なしで同じ判断ができる。
