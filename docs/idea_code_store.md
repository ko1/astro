...
**2 実装の違いは束縛先だけ**である (名前は片方が手段、片方が結果で少しずれるが、
それぞれの側で一番はっきりした語を採った)。上流 — `node.def`、生成 `.c`、hash、束縛記述子、
`astro_hole_walk` — は完全に共通で、分かれるのは `runtime/astro_hole.h` の `#if` 一枚:
...
コード内部の識別子は歴史的に穴 (`astro_hole_*`, `HOLE_U64`) を使っている。これは
パッチ束縛の視点の語 (コードに開いた空白) なので、**内部実装の語としてはそのまま残し、
説明はロード時ノード束縛で統一する**。間接束縛のコードには空白は無く、`P[k]` という
ロードがあるだけなので、穴は 2 実装を束ねる名前としては使わない。
