...
`lib/astrogen.rb` (約500行の Ruby スクリプト) が、`node.def` から以下を自動生成:

| 生成ファイル | 内容 |
|---|---|
| `node_head.h` | AST ノードの構造体定義（union でまとめる） |
...
各ノード型は `NodeKind` 構造体を持ち、デフォルトディスパッチャ・ハッシュ関数・特化関数・ダンプ関数へのポインタを格納。
...
/home/ko1/ruby/astro/
├── lib/
│   └── astrogen.rb           # ASTroGen コア (約500行 Ruby)
├── sample/
│   ├── calc/                 # 最小例 (3ノード: num, add, mul)
│   │   └── node.def
│   └── naruby/               # Ruby サブセット (21ノード)
