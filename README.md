...
- [`asml`](./sample/asml/) — **Standard ML** subset (82 nodes) with **full Hindley–Milner type inference** (Algorithm W + level-based generalisation + value restriction), ADTs with type parameters, pattern match, exceptions, refs.  Type errors halt at compile time; the type-checker drives lower-time dispatcher specialisation, and **generic dynamic-typecheck nodes are removed from `node.def` entirely** — only `_int` / `_real` / `_string` / `_poly` / `_bool` / `_unchecked` variants remain.  vs SML/NJ v110.79 native: still ~14–40× slower (asml AOT goes through AST dispatch; SML/NJ emits native code) but `_int` specialisation is comprehensive — every reachable arithmetic / comparison / control op in compiled SDs is type-specialised.
