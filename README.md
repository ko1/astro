...
- [`sample/pystro/`](./sample/pystro/) — Python 3 subset (~60 nodes). GMP bignum, classes + single-inheritance, `try`/`except`/`finally`/`raise`, `lambda` + default args, list / tuple / dict + slicing, f-strings, `for x in iter`, ~25 built-ins. Inline-cached gref / method-resolve / for-loop targets, `py_apply` inlined into SD bodies, leaf-func alloca frames, dict identity-equal fast path, share-buffer string slices. **Beats CPython 3.12 on all 6 micro-benches** at ~1 s scale: `fib(35)` 1.9×, `while_loop` (10 M) 18×, `for_range` (15 M sum) 12.5×, `list` (7 M append+sum) 4.9×, `string.split` (2 M) 1.2×, `dict` (3 M put+get) 1.04×.
