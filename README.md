...
- [`sample/asom/`](./sample/asom/) — Smalltalk dialect ([SOM](https://som-st.github.io/)). Type-specialized sends, control-flow inlining, frame pool, bump arenas, flonum-tagged Double, parse-time NLR analysis. Bundles SOM-st/SOM submodule for stdlib + TestSuite + AreWeFastYet; passes 216/221 (97.7%) TestSuite. AOT-cached beats SOM++ on all 12 AreWeFastYet (Sieve **20×**, Mandelbrot **48×**, QuickSort 10×, Queens 9×); also wins **8/12 vs TruffleSOM warm peak**, with Mandelbrot **15× faster than Truffle**.
