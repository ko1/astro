...
- [`sample/aforth/`](./sample/aforth/) — Forth subset. Each word is a NODE, `: word ;` bodies + toplevel are AOT entries; word calls indirect through a `word_id` → `NODE *` table so recursion and mutual recursion need no cycle break. AOT speedups range from 2.9× on recursion-bound benches (fib / ack / factorial — capped by the indirect-call floor) up to ~18× on inner-loop benches (collatz / sieve / tak) where the body folds into a single SD.
