...
- [`sample/asom/`](./sample/asom/) — Smalltalk dialect ([SOM](https://som-st.github.io/)). Type-specialized sends, control-flow inlining, frame pool, bump arenas, flonum-tagged Double, parse-time NLR analysis. Bundles SOM-st/SOM submodule for stdlib + TestSuite + AreWeFastYet; passes 216/221 (97.7%) TestSuite. AOT-cached beats SOM++ on all 12 AreWeFastYet (Sieve **20×**, Mandelbrot **47×**, QuickSort 10×, Queens 8×); PG-baked wins **11/12 vs TruffleSOM warm peak** (only loses Towers 1.16×), with Mandelbrot **15× faster than Truffle**.
