...
- [`sample/koruby/`](./sample/koruby/) — *kind of Ruby*: standalone (non-CRuby) Ruby implementation. Boehm GC + GMP Bignum + Prism. CRuby-compatible `VALUE` encoding so heap layouts and code can be ported. Closures (yield-shared-fp), classes/modules/include, attr_*, lexical constants, `case/when`, multi-assign, `super`, `ensure`, `require/require_relative`, basic `Struct`/`File`/`STDOUT`. Beats CRuby (no JIT) on fib by ~1.6× via interp + 3.6× with AOT specialization; ~1.6× off `--yjit` on tight benches. optcarrot is partially loadable (every file parses + module/class def chain runs); full execution still in progress — many runtime methods missing.
