...
- [`sample/jstro/`](./sample/jstro/) — JavaScript (broad ES2023 subset). V8-style hidden-class objects with shape-transition ICs, monomorphic call ICs, longjmp `throw`, safepoint-driven mark-sweep GC, JsObject inline 4-slot storage, AOT/PG specialization via `astro_code_store` (geo-mean ~2.5× over the plain interpreter). ~7 K hand-written lines.
