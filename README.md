...
- [`sample/castro/`](./sample/castro/) — **C subset on ASTro**.  tree-sitter-c front-end + Ruby parse driver (`parse.rb`) that lowers preprocessed C into a typed S-expression IR; ~96 node kinds carry the runtime side (int + double arithmetic and comparisons, pointer load / store / `ptr_add` / `ptr_diff`, structured break / continue with parse-time routing to a 3-tier loop family `*_brk` / `*_brk_only` / no-jmp, tail-return-lifted function bodies that bypass `setjmp` entirely when no early `return` survives).  8-byte VALUE union (`int64_t .i / double .d / void *.p`); every datum — int / char / pointer / array element / struct field / string literal byte — occupies one slot, and pointer arithmetic is slot-unit (`p+1` ≡ +8 byte) for internal consistency.  Globals (with initializers and designated initializers), local arrays (with deduced size), structs (`s.f` / `s->f`, nested + designated init), function pointers, `goto` (top-level labels — see todo), full `printf` family with width / precision / `l` / `ll` / `z` length modifiers (slot-string ↔ host C string lowered at the `%s` boundary), libc-ish builtins (`malloc` / `free` / `calloc` / `strlen` / `strcmp` / `strncmp` / `strcpy` / `strncpy` / `strcat` / `memset` / `memcpy` / `atoi` / `exit` / `abs`), and `gcc -E` driven preprocessing so `#include`/`#define`/`#if` work out of the box.  Three execution modes — interpreter, AOT first-build, AOT cached (dlopen of `code_store/all.so`) — wired so that `@ref` callcache + dispatcher snapshot + `-rdynamic` linkage let the SD shards inline the recursive call directly with no `castro_call_cache_fill` round-trip on the hot path.  AOT-cached **beats gcc -O0 on `loop_sum` (5 ms vs 10 ms, 2× faster)** and stays within 1.3–2.5× of gcc -O0 on `fib_big` / `fib_d` / `tak` / `ackermann` / `mandelbrot_count`.  Passes **30/30** feature tests across all three modes and **173/220 (78.6%)** of the c-testsuite.  Remaining gaps are mostly union / bit-field / variadic user functions / `_Generic` / structured-goto (forward goto into a loop body); see [`sample/castro/docs/done.md`](./sample/castro/docs/done.md) / [`todo.md`](./sample/castro/docs/todo.md) and [`runtime.md`](./sample/castro/docs/runtime.md) for the inventory and runtime architecture.
