...
- [`sample/asom/`](./sample/asom/) — Smalltalk dialect ([SOM](https://som-st.github.io/)). Type-specialized sends, control-flow inlining, frame pool, bump arenas, flonum-tagged Double, parse-time NLR analysis. Bundles SOM-st/SOM submodule for stdlib + TestSuite + AreWeFastYet; passes 216/221 (97.7%) TestSuite. AOT-cached beats SOM++ on all 12 AreWeFastYet (Sieve **19×**, Mandelbrot **3×**, QuickSort / Queens 8-10×); on Mandelbrot also **13× faster than TruffleSOM (warm peak)**.
