...
- [`sample/asom/`](./sample/asom/) — Smalltalk dialect ([SOM](https://som-st.github.io/)). Bundles SOM-st/SOM as a submodule for stdlib + TestSuite + AreWeFastYet. Passes 216/221 (97.7%) TestSuite assertions; 16 AWFY benchmarks pass. After type-specialized int/array sends, control-flow inlining (`ifTrue:` / `whileTrue:` / `to:do:` etc.), per-bucket frame pool, double / block bump arenas, and parse-time-detected no-NLR methods skipping `setjmp`, asom **beats SOM++** (`USE_TAGGING` + COPYING GC, g++ -O3 -flto) at sustained workloads — 4× faster on Sieve / Queens, 3× on Bounce, 2-2.5× on Fannkuch / BubbleSort / Permute / List / QuickSort, ties on TreeSort / Mandelbrot. (At sub-100 ms runs SOM++ wins on the small benches because per-call setup dominates; at ~1-second sustained runs asom's optimized steady-state pulls ahead.)
